<div id=<?php echo $module['module_name'];?>  class="" monxin-module="<?php echo $module['module_name'];?>" align=left save_name="<?php echo $module['module_save_name'];?>"  >
    ...
    $(document).ready(function(){
		...
		$("#<?php echo $module['module_name'];?>_html .close_comment").click(function(){$("#comment_input_div").css('display','none');});
		...
		$(document).on('click',"#<?php echo $module['module_name'];?> .comment_button",function(){
			if(getCookie('monxin_nickname')==''){window.location.href='./index.php?index.login';}
				...
				$("#<?php echo $module['module_name'];?> #comment_input_div").css('display','block').css('top',position.top-10);
				if($("#<?php echo $module['module_name'];?> #comment_input_div").offset().top!=position.top){
					...
					$("#<?php echo $module['module_name'];?> #comment_input_div").css('display','block').css('top',position.top+distance+10);
				}
    ...
    function del(id){
		...
		$.get('<?php echo $module['action_url'];?>&act=del',{cid:id}, function(data){
			...
			$("#<?php echo $module['module_name'];?> #state_"+id).html(v.info);
			if(v.state=='success'){
				$("#content_"+id).animate({opacity:1},"slow",function(){$("#content_"+id).css('display','none');});
			}
		});        	
        return false; 
    }
	...
	#<?php echo $module['module_name'];?>_html .content_div .content_div{ margin-bottom:1rem; border-bottom:1px dashed #ccc;}
	#<?php echo $module['module_name'];?>_html .content_div .buttons{ opacity:0.3; margin-top:0.5rem;}
	...
	#<?php echo $module['module_name'];?>_html .content_div .buttons a:hover{ opacity:0.3;}
	#<?php echo $module['module_name'];?>_html .content_div:hover .comment_div .buttons2 a{ opacity:0.3;  border-radius:0.3rem;  }
	#<?php echo $module['module_name'];?>_html .content_div:hover .comment_div .buttons2  a:hover{ opacity:0.3;}
	...
	#<?php echo $module['module_name'];?>_html .content_div .comment_div .buttons2{ opacity:0.3; margin-right:1rem;}
	#<?php echo $module['module_name'];?>_html .content_div .comment_div{ text-align:left;  background:#fff;}
   ...
   #<?php echo $module['module_name'];?>_html .content_div .comment_div .username:before{font: normal normal normal 1rem/1 FontAwesome; content:"\f007"; padding-right:2px;}
	...
	#<?php echo $module['module_name'];?>_html .content_div .comment_div .username{}
	#<?php echo $module['module_name'];?>_html .content_div .comment_div .time{ }

	
    #<?php echo $module['module_name'];?>_html .content_div  .content_div{ border-bottom:1px dashed <?php echo $_POST['monxin_user_color_set']['nv_1_hover']['background']?>; padding-bottom:5px;  background:#fff;}
	#<?php echo $module['module_name'];?>_html .content_div .comment_div{ padding-left:1rem;}
    ...
    #<?php echo $module['module_name'];?>_html #comment_input_div{ display:none; position:absolute; background:#fff;box-shadow: 1px 1px 8px 0px rgba(0,0,0,.3); line-height:3rem; padding:5px;}
	...
	#<?php echo $module['module_name'];?>_html .replay_a_div .replay{  display:inline-block;    border-radius:5px; padding:5px;border:1px solid <?php echo $_POST['monxin_user_color_set']['nv_2_hover']['background']?>; border-radius:3px; background:none; color:<?php echo $_POST['monxin_user_color_set']['nv_2_hover']['background']?>;}
	...
	#<?php echo $module['module_name'];?>_html .replay_a_div .replay:hover{ opacity:0.3;}
	...
	#<?php echo $module['module_name'];?>_html .close_comment:before{font: normal normal normal 1rem/1 FontAwesome; content:"\f00d";}
    </style>
    ...
    <div id=comment_input_div><span class=m><input type=text id=comment_input maxlength="300" size="40" placeholder="<?php echo self::$language['content'];?>" /><br /><input type=text id=comment_auth placeholder="<?php echo self::$language['authcode'];?>" /> <img id="comment_auth_img" wsrc="./lib/authCode.class.php?save_name=comment_auth" style="vertical-align:middle; border:0px;" /> <a href=# onclick="return submit_comment()" class="submit"><?php echo self::$language['submit'];?></a> <a class=close_comment></a><span id=comment_state></span></span><span id=comment_input_div_close class="e"> </span>
    </div>
