...
$(document).ready(function(){
	...
	$("#<?php echo $module['module_name'];?> .distribution").click(function(){
		id=$(this).attr('d_id');
		$(this).next().html('<span class=\'fa fa-spinner fa-spin\'></span><?php echo self::$language['executing'];?>');
		$.post('<?php echo $module['action_url'];?>&act=distribution',{id:id} ,function(data){
			//alert(data);
...
.distribution{color:#fff !important;}
.distribution_info{}
.distribution_info a{ display:inline-block; vertical-align:top; background-color:#F60; color:#fff; border-radius:0.5rem; padding-left:0.5rem; padding-right:0.5rem; line-height:1.5rem;}
.distribution_info a:hover{ opacity:0.8;}

#<?php echo $module['module_name'];?>_html .price_span{ color:#F00; line-height:2rem;}
#<?php echo $module['module_name'];?>_html .already_distribution{ padding-left:0.5rem; color:#999;}
</style>
