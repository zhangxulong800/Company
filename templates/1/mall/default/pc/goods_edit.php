<div id=<?php echo $module['module_name'];?>  class="portlet light" monxin-module="<?php echo $module['module_name'];?>" align=left  >
	...
	function set_goods_value(v){
		if(v.name &&　get_param('old_title')==''){$("#<?php echo $module['module_name'];?> #title").val(v.name);}
	...
	function goods_move(){
		...
		$.post("<?php echo $module['action_url'];?>&act=goods_move",{move_url:$(".fast_move .move_url").val()},function(data){
			...
			if(v.state=='success'){
				$(".fast_move").animate({width:120,height:30},'fast',function(){$(".fast_move").css('background','none');});
				set_goods_value(v);
    ...
    $(document).ready(function(){
		...
		if(get_param('c_url')){
			$(".fast_move .move_url").val(get_param('c_url'));
			setTimeout("goods_move()",2000);
		}
		if(get_param('auto_submit')=='1'){setTimeout("goods_submit()",20000);}
		
		$(".fast_move .fast_move_switch").click(function(){
			//alert($(".fast_move").parent().width());
			if($(".fast_move").width()<120){
				$(".fast_move").animate({width:500,height:180},'fast');
				$(".fast_move").css('background-color','#CCC');
			}else{
				$(".fast_move").animate({width:120,height:50},'fast',function(){$(".fast_move").css('background','none');});
		...
		$("#<?php echo $module['module_name'];?> .search_same").attr('href',$("#<?php echo $module['module_name'];?> .search_same").attr('href_pre')+$("#<?php echo $module['module_name'];?> #title").val());
		
		$("[monxin_value]").each(function(index, element) {
            $(this).val($(this).attr('monxin_value'));
        });
		$("[d=goods_state_"+$("#goods_state").val()+"]").css('display','block');
		if($("#limit").val()!=0){$("[d=buy_limit]").css('display','block');}
		
		$("#limit").change(function(){
			if($(this).val()!=0){
				$("[d=buy_limit]").css('display','block');
				if($("#<?php echo $module['module_name'];?> #limit_cycle").val()==null){$("#<?php echo $module['module_name'];?> #limit_cycle").val('d');}
			}else{
		...
		if($("#<?php echo $module['module_name'];?> #deposit").val()!='' && <?php echo $module['pre_sale']?>==1){
			$("#<?php echo $module['module_name'];?> #pre_discount").attr('disabled','disabled');	
			$("#<?php echo $module['module_name'];?> #deposit").attr('disabled','disabled');	
			$("#<?php echo $module['module_name'];?> #reduction").attr('disabled','disabled');	
		}
		...
		if(''!='<?php echo $module['data']['bar_code'];?>'){
			$("#<?php echo $module['module_name'];?> .bar_code_img").attr('src','./plugin/barcode/buildcode.php?codebar=BCGcode128&text=<?php echo $module['data']['bar_code'];?>');	
		}
		...
		$(document).on('click',"#<?php echo $module['module_name'];?> .del_color_img",function(){
			...
			$("#<?php echo $module['module_name'];?> #"+$(this).attr('input_id').replace(/_td/,'')).val('del');
			return false;
		});
		...
		var temp_tag='<?php echo $module['data']['shop_tag']?>';
		if(temp_tag!=''){
			temp=temp_tag.split('|');
			for(v in temp){
		...
		<?php echo $module['set_attribute'];?>
		$('#icon_ele').insertBefore($('#icon_state'));
		$('#multi_angle_img_ele').insertBefore($('#multi_angle_img_state'));
		
		if(''!='<?php echo $module['data']['multi_angle_img']?>'){
			$('#multi_angle_img').val('<?php echo $module['data']['multi_angle_img']?>');
			submit_hidden('multi_angle_img');	
		}
		...
		$("#<?php echo $module['module_name'];?> input").keydown(function(event){
			if(event.keyCode==13){return goods_submit();}		  	
		});
		<?php echo $module['set_specifications'];?>
		...
		$("#<?php echo $module['module_name'];?>_html .bar_code").each(function(index, element) {
            if($(this).val()==0){$(this).val('');}
        });
		
		$("#<?php echo $module['module_name'];?> #option_table .bar_code").each(function(index, element) {
            if($(this).val()!=''){
		...
		$("#<?php echo $module['module_name'];?> input[class='color_name']").each(function(index, element) {
			id=$(this).attr('id').replace(/color_name_/,'');
            $("#<?php echo $module['module_name'];?> .color_"+id+" .color_name").html($(this).val());
        });
		
		
		$("#<?php echo $module['module_name'];?> #option_table tr").each(function(index, element) {
            if($(this).css('display')=='table-row'){
				id=$(this).attr('id');
				$("#<?php echo $module['module_name'];?> #"+id+" .quantity").css('display','none');
				$("#<?php echo $module['module_name'];?> #"+id+" .quantity").parent().append($("#<?php echo $module['module_name'];?> #"+id+" .quantity").val());
				$("#<?php echo $module['module_name'];?> #"+id+" .cost_price").css('display','none');
				$("#<?php echo $module['module_name'];?> #"+id+" .cost_price").parent().append($("#<?php echo $module['module_name'];?> #"+id+" .cost_price").val());
				
			}
        });
		
		setTimeout('set_after_css()',1000);
    });
    
	function set_after_css(){
		var after_css='';
		$("#<?php echo $module['module_name'];?> .e_price").each(function(index, element) {
           after_css+="[value='"+$(this).val()+"']:after{content:'"+$(this).val()/0.1+"'}";
        });
		
		after_css='<style>'+after_css+'</style>';
		$(".after_css").html(after_css);
	}
    ...
    function goods_submit(){
		...
		if($("#<?php echo $module['module_name'];?> #discount").val()==''){
			$("#<?php echo $module['module_name'];?> #discount").next('.state').html('<span class=fail><?php echo self::$language['please_select']?></span>');
			$("#<?php echo $module['module_name'];?> #discount").focus();
			is_null=true;return false;
		}	
		if(!$.isNumeric($("#<?php echo $module['module_name'];?> #discount").val())){
			$("#<?php echo $module['module_name'];?> #discount").next('.state').html('<span class=fail><?php echo self::$language['must_be']?><?php echo self::$language['number']?></span>');
			$("#<?php echo $module['module_name'];?> #discount").focus();
			is_null=true;return false;
		}	
		if($("#<?php echo $module['module_name'];?> #discount").val()<10){
			if($("#<?php echo $module['module_name'];?> #discount_start_time").val()==''){
				$("#<?php echo $module['module_name'];?> #discount_start_time").next('.state').html('<span class=fail><?php echo self::$language['please_select']?></span>');
				$("#<?php echo $module['module_name'];?> #discount_start_time").focus();
				is_null=true;return false;
			}	
			if($("#<?php echo $module['module_name'];?> #discount_end_time").val()==''){
				$("#<?php echo $module['module_name'];?> #discount_end_time").next('.state').html('<span class=fail><?php echo self::$language['please_select']?></span>');
				$("#<?php echo $module['module_name'];?> #discount_end_time").focus();
				is_null=true;return false;
			}	
		}
		  ...
		  if(!$.isNumeric(obj['bar_code']) && obj['bar_code']!=''){$("#<?php echo $module['module_name'];?> #bar_code").next().html('<span class=fail><?php echo self::$language['must_be']?><?php echo self::$language['number']?></span>');$("#<?php echo $module['module_name'];?> #bar_code").focus();return false;}
			...
			$("#<?php echo $module['module_name'];?> #option_table tbody tr").each(function(index, element) {
				if($(this).css('display')!='none'){
					...
					//alert(obj['specifications'][$(this).attr('id')]);
				}
            });
		...
		obj['discount']=$("#<?php echo $module['module_name'];?> #discount").val();	
		obj['discount_start_time']=$("#<?php echo $module['module_name'];?> #discount_start_time").val();	
		obj['discount_end_time']=$("#<?php echo $module['module_name'];?> #discount_end_time").val();	
		obj['virtual_auto_delivery']=$("#<?php echo $module['module_name'];?> #virtual_auto_delivery").val();	
	...
	function submit_hidden(id){
		...
		if(id=='icon'){
			...
			multi_angle_img_old='<?php echo $module['data']['multi_angle_img'];?>';
			$("#multi_angle_img_fieldset_succeedList").css('display','none');
			temp=v.split('|');
			temp_html='';
			for(i in temp){
				if(temp[i]!=''){
					if(multi_angle_img_old.indexOf(temp[i])!=-1 || temp[i].indexOf('./program/mall/img/')!=-1){
						if(temp[i].indexOf('./program/mall/img/')!=-1){
							temp_html+='<div><img src="'+temp[i]+'" /><br /><a href=# class=move_to_left>&nbsp;</a><a href=# class=move_to_right>&nbsp;</a><a href=# class=delete_me>&nbsp;</a></div>';	
						}else{
							temp_html+='<div><img src="./program/mall/img/'+temp[i]+'" /><br /><a href=# class=move_to_left>&nbsp;</a><a href=# class=move_to_right>&nbsp;</a><a href=# class=delete_me>&nbsp;</a></div>';	
						}
			...
			$("#multi_angle_img_show .delete_me").click(function(){
				img_path=$(this).parent().children('img').attr('src');
				$(this).parent().remove();
				multi_angle_img_val_reset()
				return false;	
			});
			$("#multi_angle_img_show .move_to_left").click(function(){
				a=$(this).parent();
				b=a.prev();
				a.insertBefore(b);
				multi_angle_img_val_reset()
			...
			$("#multi_angle_img_show .move_to_right").click(function(){
				a=$(this).parent();
				b=a.next();
				a.insertAfter(b);
				multi_angle_img_val_reset()
	...
	#<?php echo $module['module_name'];?> .require{}
	...
	#<?php echo $module['module_name'];?>_html .option_span{  padding-right:20px; }
	...
	#color_info_table thead tr td{  padding-left:2%; padding-right:2%;  font-size:0.8rem; white-space:nowrap;}
	#color_info_table tbody tr td{ border-bottom:1px solid #eee;  padding-left:2%;white-space:nowrap;}
	...
	#<?php echo $module['module_name'];?> .quick_set a{ margin-right:20px;}
	#<?php echo $module['module_name'];?> .quick_set a:hover{ opacity:0.8;}
	</style>
    <div id="<?php echo $module['module_name'];?>_html">
    
    <form id="monxin_form" name="monxin_form" method="POST" action="<?php echo $module['action_url'];?>" onSubmit="return exe_check();">
    	<div class=line_div>
        	<span class=m_label><span id=type><?php echo self::$language['belong'];?><?php echo self::$language['mall'];?><?php echo self::$language['type'];?></span></span>
        	<span class=input_span><?php echo $module['type']?><span class=state></span> <a href="./index.php?monxin=mall.goods_add_option_type&act=edit&id=<?php echo $_GET['type'];?>&goods_id=<?php echo $_GET['id'];?>" class="edit"><?php echo self::$language['edit'];?></a></span>
        </div>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span><select id=shop_type name=shop_type monxin_value='<?php echo $module['data']['shop_type'];?>'><option value=0>&nbsp;</option><?php echo $module['shop_type']?></select><input type="text" class="hide_input" /> <span class=state></span><a href="#" class=switch_input>&nbsp;</a>
            </span>
        </div>
    	<div class=line_div>
        	<span class=m_label><?php echo self::$language['brand'];?></span>
        	<span class=input_span>
            	<select id="brand" name="brand" monxin_value='<?php echo $module['data']['brand'];?>'>
      ...
      <?php echo $module['brand_option'];?></select><input type="text" class="hide_input" /> <span class=state></span><a href="#" class=switch_input>&nbsp;</a>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="grade" name="grade" monxin_value='<?php echo $module['data']['grade'];?>'>
      <option value="-1">&nbsp;</option> 
      <?php echo $module['grade_option'];?></select><input type="text" class="hide_input" /> <span class=state></span><a href="#" class=switch_input>&nbsp;</a>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="habitat" name="habitat" monxin_value='<?php echo $module['data']['habitat'];?>'>
      <option value="-1">&nbsp;</option> 
      <?php echo $module['habitat_option'];?></select><input type="text" class="hide_input" /> <span class=state></span><a href="#" class=switch_input>&nbsp;</a>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="contain" name="contain" monxin_value='<?php echo $module['data']['contain'];?>'>
      <option value="-1">&nbsp;</option> 
      <?php echo $module['contain_option'];?></select><input type="text" class="hide_input" /> <span class=state></span><a href="#" class=switch_input>&nbsp;</a>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="position" name="position" monxin_value='<?php echo $module['data']['position'];?>'>
      <option value="-1">&nbsp;</option> 
      <?php echo $module['position'];?></select><input type="text" class="hide_input" /> <span class=state></span><a href="#" class=switch_input>&nbsp;</a>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="storehouse" name="storehouse" monxin_value='<?php echo $module['data']['storehouse'];?>'>
      <option value="-1">&nbsp;</option> 
      <?php echo $module['storehouse'];?></select><input type="text" class="hide_input" /> <span class=state></span><a href="#" class=switch_input>&nbsp;</a>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="supplier" name="supplier" monxin_value='<?php echo $module['data']['supplier'];?>'>
      <option value="-1">&nbsp;</option> 
      <?php echo $module['supplier'];?></select><input type="text" class="hide_input" /> <span class=state></span><a href="#" class=switch_input>&nbsp;</a>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="unit" name="unit" monxin_value='<?php echo $module['data']['unit'];?>'>
      <option value="-1">&nbsp;</option> 
      <?php echo $module['unit'];?></select><input type="text" class="hide_input" /> <span class=state></span><a href="#" class=switch_input>&nbsp;</a>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span><input type="text" name="title" id="title" value='<?php echo $module['data']['title'];?>'  /> <span class=state></span> <a  class="search_same" user_color="button" style="padding:5px; line-height:20px;" href=# href_pre="https://search.jd.com/Search?enc=utf-8&keyword=" target="_blank"><?php echo self::$language['search_same']?></a></span>
        </div>
		    
    	<div class=line_div>
        	<span class=m_label><?php echo self::$language['goods_advantage'];?></span>
        	<span class=input_span><textarea type="text" name="advantage" id="advantage"><?php echo $module['data']['advantage'];?></textarea> <span class=state></span></span>
        </div> 
    	<div class=line_div>
        	<span class=m_label><?php echo self::$language['tag'];?></span>
        	<span class=input_span><input type="hidden" id="tag" name="tag"  value='<?php echo $module['data']['tag'];?>'/><?php echo $module['tags'];?>  <span class=state></span><a href="index.php?monxin=mall.s_tag" class=create><?php echo self::$language['create']?></a>
            </span>
        </div>
    	<div class=line_div>
        	<span class=m_label><?php echo self::$language['cover_image'];?></span>
        	<span class=input_span><span class=state id=icon_state></span>
            <div id=cover_image_show><img src='./program/mall/img_thumb/<?php echo $module['data']['icon'];?>' /></div></span>
        </div>
     ...
     <!-- -->  
        ...
        <div id=no_specifications style="display:none;">
            <div class=line_div>
                <span class=m_label><?php echo self::$language['bar_code'];?></span>
                <span class=input_span><input type="text" name="bar_code" id="bar_code" value='<?php echo $module['data']['bar_code'];?>' class="bar_code" maxlength="13" /> <span class=state></span>  <img class=bar_code_img /></span>
                
            </div>
                    
            <div class=line_div>
                <span class=m_label><?php echo self::$language['store_code'];?></span>
                <span class=input_span><input type="text" name="store_code" id="store_code" value='<?php echo $module['data']['store_code'];?>' class="store_code" maxlength="15" /> <span class=state></span>  <img class=store_code_img /></span>
                
            </div>
             
            <div class=line_div>
                <span class=m_label><?php echo self::$language['cost_price'];?></span>
                <span class=input_span><?php echo $module['data']['cost_price'];?></span>
            </div>
			 
            <div class=line_div>
                <span class=m_label><?php echo self::$language['w_price'];?><span class=require>*</span></span>
                <span class=input_span><input type="text" name="w_price" id="w_price" value='<?php echo $module['data']['w_price'];?>' /> <span class=state></span></span>
            </div>
    	...
    	<div class=line_div><span class=m_label><?php echo self::$language['general'];?><?php echo self::$language['discount_rate'];?><span class=require>*</span></span><span class=input_span><input type="text" id="discount"  name="discount" value="<?php echo $module['data']['discount'];?>"  /> <span class=state></span></span></div>
    	<div class=line_div><span class=m_label><?php echo self::$language['general'];?><?php echo self::$language['discount_start_time'];?></span><span class=input_span><input type="text" name="discount_start_time" id="discount_start_time" class='discount_start_time' onclick=show_datePicker(this.id,'date_time') onblur= hide_datePicker() value="<?php echo $module['data']['discount_start_time'];?>"  /> <span class=state></span></span></div>
    	<div class=line_div><span class=m_label><?php echo self::$language['general'];?><?php echo self::$language['discount_end_time'];?></span><span class=input_span><input type="text" name="discount_end_time" id="discount_end_time" class='discount_end_time' onclick=show_datePicker(this.id,'date_time') onblur= hide_datePicker() value="<?php echo $module['data']['discount_end_time'];?>"  /> <span class=state></span></span></div>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="goods_state" name="goods_state" monxin_value="<?php echo $module['data']['state']?>"><?php echo $module['goods_state']?></select> <span class=state></span>
            </span>
        </div>
    	...
    	<div class=line_div d="goods_state_1" style="display:none;">
        	...
        	<span class=input_span style="border:1px #CCCCCC solid;">
            	<div class=sub_line_div>
                	<span class=m_label><?php echo self::$language['booking'];?><?php echo self::$language['rebate'];?><span class=require>*</span></span><span class=input_span><input type="text" class="pre_discount" id="pre_discount" name="pre_discount" value="<?php echo $module['data']['pre_discount']?>"  /><span class=state></span></span>
                	<span class=m_label><?php echo self::$language['booking'];?><?php echo self::$language['deposit2'];?><span class=require>*</span></span><span class=input_span><input type="text" class="deposit" id="deposit" name="deposit" value="<?php echo $module['data']['deposit']?>" />  <?php echo self::$language['yuan']?> <span class=state></span></span>
                	<span class=m_label><?php echo self::$language['deposit2'];?><?php echo self::$language['reduce'];?><span class=require>*</span></span><span class=input_span><input type="text" class="reduction" id="reduction" name="reduction" value="<?php echo $module['data']['reduction']?>" /> <?php echo self::$language['yuan']?> <span class=state></span></span>
                	<span class=m_label><?php echo self::$language['last_pay_start_time'];?><span class=require>*</span></span><span class=input_span><input type="text" id="last_pay_start_time" name="last_pay_start_time" class="last_pay_start_time" value="<?php echo $module['data']['last_pay_start_time']?>" onclick=show_datePicker(this.id,'date') onblur= hide_datePicker()  /><span class=state></span></span>
                	<span class=m_label><?php echo self::$language['last_pay_end_time'];?><span class=require>*</span></span><span class=input_span><input type="text" id="last_pay_end_time" name="last_pay_end_time" class="last_pay_end_time" value="<?php echo $module['data']['last_pay_end_time']?>" onclick=show_datePicker(this.id,'date') onblur= hide_datePicker()  /><span class=state></span></span>
                	<span class=m_label><?php echo self::$language['delivered'];?><span class=require>*</span></span><span class=input_span><input type="text" id="delivered" name="delivered" value="<?php echo $module['data']['delivered']?>" onclick=show_datePicker(this.id,'date') onblur= hide_datePicker()  /><span class=state></span></span>
                </div>
    	...
    	<div class=line_div d="goods_state_3" style="display:none;">
        	...
        	<span class=input_span><input type="text" name="out_delivered" id="out_delivered"  value="<?php echo $module['data']['out_delivered']?>" onclick=show_datePicker(this.id,'date') onblur= hide_datePicker()  /> <span class=state></span></span>
        </div>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="limit" name="limit" monxin_value="<?php echo $module['data']['limit']?>"><?php echo $module['buy_limit_option']?></select> <span class=state></span>
            </span>
        </div>
		    
    	<div class=line_div d="buy_limit" style="display:none;">
        	<span class=m_label><?php echo self::$language['buy_limit_cycle'];?></span>
        	<span class=input_span><select id="limit_cycle" name="limit_cycle"  monxin_value="<?php echo $module['data']['limit_cycle']?>"><?php echo $module['limit_cycle_option']?></select> <span class=state></span></span>
        </div>
		       
    	<div class=line_div>
        	<span class=m_label><?php echo self::$language['free_shipping'];?></span>
        	<span class=input_span>
            	<select id="free_shipping" name="free_shipping"  monxin_value='<?php echo $module['data']['free_shipping'];?>'><option value="0"><?php echo self::$language['no']?></option><option value="1"><?php echo self::$language['yes']?></option></select> <span class=state></span>
            </span>
        </div>
		<div class=line_div>
        	<span class=m_label>商品类型</span>
        	<span class=input_span>
            	<select id="goods_type" name="goods_type"  monxin_value='<?php echo $module['data']['goods_type'];?>'>
		...
		<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="only_integral" name="only_integral"  monxin_value='<?php echo $module['data']['only_integral'];?>'>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span>
            	<select id="sales_promotion" name="sales_promotion"  monxin_value='<?php echo $module['data']['sales_promotion'];?>'><option value="1"><?php echo self::$language['yes']?></option><option value="0"><?php echo self::$language['no']?></option></select> <span class=state></span>
            </span>
        </div>
    	...
    	<div class=line_div>
        	...
        	<span class=input_span><input type="text" name="logistics_weight" id="logistics_weight" value='<?php echo $module['data']['logistics_weight'];?>' /> <span class=state></span></span>
        </div>
		    
    	<div class=line_div>
        	<span class=m_label><?php echo self::$language['logistics_volume'];?></span>
        	<span class=input_span><input type="text" name="logistics_volume" id="logistics_volume" value='<?php echo $module['data']['logistics_volume'];?>' /> <span class=state></span></span>
        </div>

    	<div class=line_div>
        	<span class=m_label><?php echo self::$language['shelf_life'];?></span>
        	<span class=input_span><input type="text" name="shelf_life" id="shelf_life" value='<?php echo $module['data']['shelf_life'];?>' /> <span class=state></span></span>
        </div>
    	...
    	<div class=line_div>
            ...
            <textarea type="text" name="virtual_auto_delivery" id="virtual_auto_delivery"  placeholder="<?php echo self::$language['virtual_auto_delivery_placeholder'];?>"><?php echo $module['data']['virtual_auto_delivery'];?></textarea> <span class=state></span></span>
        </div>
    ...
    <ul class="nav nav-tabs" device_tabs=1>
      ...
      <li role="presentation"><a href="#m_detail" target="m_detail_div"><?php echo self::$language['phone_device'];?><?php echo self::$language['detail'];?></a></li>
		...
		<div class="detail_div"><textarea name="detail" id="detail" style="display:none; width:100%; height:400px;"><?php echo $module['data']['detail'];?></textarea></div>
        <div class="m_detail_div" style="display:none;"><textarea name="m_detail" id="m_detail" style="display:none; width:100%; height:400px;"><?php echo $module['data']['m_detail'];?></textarea></div>
    ...
    <style>
    .fast_move{ position:fixed; right:50px; top:260px; text-align:right; width:120px; height:50px; line-height:30px; overflow:hidden; padding:10px;}
