...
<script>
		...
		if(key==''){
			...
			$("#<?php echo $module['module_name'];?> a[py]").each(function(index, element) {
				...
				if($(this).attr('py').toLowerCase().indexOf(key.toLowerCase())!=-1 || $(this).children('.title').html().indexOf(key.toLowerCase())!=-1){
					str+='<a href='+$(this).attr('key')+'>'+$(this).children('.title').html().substring(0,30)+' <span>'+$(this).children('.icon_other').children('.option_price').children('.option').html()+'</span> '+'</a>';
				}
			});
		}
	...
	$("#<?php echo $module['module_name'];?> .search_result_div").css('top',$("#<?php echo $module['module_name'];?> .barcode").offset().top+30).css('left',$("#<?php echo $module['module_name'];?> .barcode").offset().left);
		...
		$(window).keydown(function(event){
			...
			if(event.keyCode==32 && $(event.target).attr('class')=='preferential_code'){
				update_fulfil();
				submit_checkout();
				return false;
			}
			
			if(event.keyCode==32 && $(event.target).attr('class')!=='received_money'){
		...
		$(document).on('click',"#<?php echo $module['module_name'];?> .pay_method a",function(){
			...
			if($(this).attr('value')=='shop_balance'){$("#<?php echo $module['module_name'];?> .pay_method_sub_div .balance_div").css('display','inline-block');}
        ...
        <div class="modal fade" id="shortcut_key_div" tabindex="-1" role="dialog" 
           ...
           <div class="modal-dialog">
              <div class="modal-content">
                 ...
                 <div class="modal-body" style=" text-align:center; line-height:50px;">
                    <a class=space_key><?php echo self::$language['checkout_and_print']?>：<?php echo self::$language['space_key']?>(Space)</a>
