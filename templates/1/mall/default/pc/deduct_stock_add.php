<div id=<?php echo $module['module_name'];?>  class="portlet light" monxin-module="<?php echo $module['module_name'];?>" align=left >
	...
	function exe_purchase_add(){
		...
		$.post('<?php echo $module['action_url'];?>&act=add',{id:$("#<?php echo $module['module_name'];?> .id").val(),option_id:$("#<?php echo $module['module_name'];?> #option_id").val(),supplier:$("#<?php echo $module['module_name'];?> .supplier").val(),quantity:$("#<?php echo $module['module_name'];?> .quantity").val(),reason:$("#<?php echo $module['module_name'];?> .reason_text").val()}, function(data){
			...
			if(v.state=='success'){
				...
				$("#<?php echo $module['module_name'];?> .submit").next('.state').html(v.info+' <a href=./index.php?monxin=mall.deduct_stock><?php echo self::$language['view'];?></a>');
			}
		});
	}
    ...
    <div id="<?php echo $module['module_name'];?>_html"  monxin-table=1>
    	...
    	<div class=goods_info>
            ...
            <div class=line><span class=m_label><?php echo self::$language['issue_2']?><?php echo self::$language['reason']?>:</span><span class=m_value><input type="text" class=reason_text  /><select class=reason_select><?php echo $module['loss_reason_option']?></select> <a href=# class=reason_switch></a> <span class=state></span></span></div>
            <div class=line><span class=m_label><?php echo self::$language['issue_2']?><?php echo self::$language['quantity']?>:</span><span class=m_value><input type="text" class=quantity /> <?php echo @$module['data']['unit'];?> <span class=state></span></span></div>
