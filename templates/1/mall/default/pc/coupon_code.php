<div id=<?php echo $module['module_name'];?>  class="portlet light" monxin-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		...
		$("#<?php echo $module['module_name'];?> tbody .open").change(function(){
			id=$(this).attr('d_id');
			$("#<?php echo $module['module_name'];?> #tr_"+id+" .open").next().html('<span class=\'fa fa-spinner fa-spin\'></span>');
            $.get('<?php echo $module['action_url'];?>&act=update',{id:id,open:$(this).val()}, function(data){
                try{v=eval("("+data+")");}catch(exception){alert(data);}
                $("#<?php echo $module['module_name'];?> #tr_"+id+" .open").next().html(v.info);
            });
				
		});
		
		$(document).on('click','#<?php echo $module['module_name'];?> .push,.draws,.used',function(){
    ...
    function add(){
        ...
        openNew=$("#<?php echo $module['module_name'];?> #open_new");	
        ...
        if(openNew.prop('value')==''){$("#<?php echo $module['module_name'];?> #state_new").html('<?php echo self::$language['please_input']?><?php echo self::$language['coupon_open']?>');openNew.focus();return false;}	
       ...
       if(parseFloat(min_moneyNew.val())<parseFloat(amountNew.val())){
        ...
        $.get('<?php echo $module['action_url'];?>&act=add',{name:nameNew.prop('value'),open:openNew.prop('value'),amount:amountNew.prop('value'),min_money:min_moneyNew.prop('value'),start_time:start_timeNew.prop('value'),end_time:end_timeNew.prop('value'),sum_quantity:sum_quantityNew.prop('value'),join_goods:$("#<?php echo $module['module_name'];?>  #join_goods_new").val()}, function(data){
            //alert(data);
            try{v=eval("("+data+")");}catch(exception){alert(data);}
	...
	function update_draws(id,draws){
		$("#<?php echo $module['module_name'];?> #tr_"+id+" .draws").html(draws);
	}
	
    </script>
    <style>
    #<?php echo $module['module_name'];?>_html{}
    #<?php echo $module['module_name'];?>_html .name{width:100px;}
    #<?php echo $module['module_name'];?>_html .amount{width:80px;}
    #<?php echo $module['module_name'];?>_html .min_money{width:80px;}
    ...
    <div class=table_scroll><table class="table table-striped table-bordered table-hover dataTable no-footer"  role="grid" style="width:100%" cellpadding="0" cellspacing="0">
         <thead>
            <tr>
                ...
                <td><?php echo self::$language['coupon_open']?></td>
                ...
                <td><?php echo self::$language['coupon_draws']?></td>
        ...
        <tbody>
            <tr id="<?php echo $module['module_name'];?>_new">
              ...
              <td ><select id="open_new" ><option value="1"><?php echo self::$language['yes']?></option><option value="0"><?php echo self::$language['no']?></option></select></td>
