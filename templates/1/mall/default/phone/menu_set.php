<div id=<?php echo $module['module_name'];?>  class="portlet light" monxin-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		$("#<?php echo $module['module_name'];?> .menu_switch").prop('value',<?php echo $module['phone_menu_show'];?>);
		if(<?php echo $module['phone_menu_show'];?>){
			$("#<?php echo $module['module_name'];?>_html").css('display','block');
		}
		$("#<?php echo $module['module_name'];?> .menu_switch").change(function(){
			if($(this).val()==1){
				$("#<?php echo $module['module_name'];?>_html").css('display','block');
			}else{
				$("#<?php echo $module['module_name'];?>_html").css('display','none');
			}
			$.get('<?php echo $module['action_url'];?>&act=update_phone_menu_show',{phone_menu_show:$(this).val()});
		});
...
$.get('<?php echo $module['action_url'];?>&act=del',{id:id}, function(data){
    ...
    #<?php echo $module['module_name'];?> .visible{}
	#<?php echo $module['module_name'];?> .filter{ font-size:24px; height:60px; line-height:60px;}
    #<?php echo $module['module_name'];?> #visible_filter{ height:26px;}
	#<?php echo $module['module_name'];?> #open_target_filter{ height:26px;}
	#<?php echo $module['module_name'];?> .icon{float:right; background:#ff9600; border-radius:3px; display:inline-block; width:30px; height:30px; padding:2px; margin-right:5px; display:none;}
...
#monxin_iframe{ height:100px;width:500px;}
    ...
    <fieldset><legend class=menu_switch_div><?php echo self::$language['open_menu'];?><select class=menu_switch><option value=1><?php echo self::$language['yes']?></option><option value=0><?php echo self::$language['no']?></option></select></legend>
