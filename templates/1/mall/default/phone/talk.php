<div id=<?php echo $module['module_name'];?>  class="portlet light" monxin-module="<?php echo $module['module_name'];?>" align=left >
    ...
    function add(){
        ...
        codeNew=$("#<?php echo $module['module_name'];?> #code_new");	
        ...
        if(codeNew.prop('value')==''){$("#<?php echo $module['module_name'];?> #state_new").html('<?php echo self::$language['please_input']?><?php echo self::$language['code']?>');codeNew.focus();return false;}	
        ...
        $.get('<?php echo $module['action_url'];?>',{name:nameNew.prop('value'),code:codeNew.prop('value'),sequence:sequenceNew.prop('value')}, function(data){
            //alert(data);
    ...
    function update(id){
        ...
        var code=$("#<?php echo $module['module_name'];?> #code_"+id);	
        ...
        if(code.prop('value')==''){$("#<?php echo $module['module_name'];?> #state_"+id).html('<?php echo self::$language['please_input']?><?php echo self::$language['code']?>');code.focus();return false;}	
        ...
        $.get('<?php echo $module['action_url'];?>&act=update',{name:name.prop('value'),code:code.prop('value'),sequence:sequence.prop('value'),id:id}, function(data){
            //alert(data);
            try{v=eval("("+data+")");}catch(exception){alert(data);}
    ...
    #<?php echo $module['module_name'];?>_html .code{width:600px;}
    ...
    <div class=table_scroll><table class="table table-striped table-bordered table-hover dataTable no-footer"  role="grid" style="width:100%" cellpadding="0" cellspacing="0">
         <thead>
            <tr>
                ...
                <td><?php echo self::$language['code']?></td>
        ...
        <tbody>
            ...
            <tr id="<?php echo $module['module_name'];?>_new">
              ...
              <td ><input type="text" name="code_new" id="code_new" class='code' /></td>
