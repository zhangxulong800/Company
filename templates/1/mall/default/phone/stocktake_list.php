<div id=<?php echo $module['module_name'];?>  class="portlet light" monxin-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		...
		$(document).on('click',"#<?php echo $module['module_name'];?> [monxin-table] .loss", function() {
			...
			confirm_return=confirm('<?php echo self::$language['confirm'];?><?php echo self::$language['loss'];?> '+loss+' '+$("#<?php echo $module['module_name'];?> #tr_"+id+" .unit").html());
			if(confirm_return){
				$("#<?php echo $module['module_name'];?> .bulk_loss_state"+id).html('<span class=\'fa fa-spinner fa-spin\'></span>');
		...
		$(document).on('click',"#<?php echo $module['module_name'];?> [monxin-table] .bulk_loss", function() {
			...
			if(confirm_return){
				...
				$.post('<?php echo $module['action_url'];?>&act=bulk_loss', function(data){
					...
					$("#state_"+id).html(v.info);
					if(v.state=='success'){
						$("#<?php echo $module['module_name'];?> .bulk_loss_state").html('<?php echo self::$language['order_state'][6];?>');
    ...
    <div class=table_scroll><table class="table table-striped table-bordered table-hover dataTable no-footer"  role="grid"  id="<?php echo $module['module_name'];?>_table" style="width:100%" cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                ...
                <td ><?php echo self::$language['stocktaked']?><?php echo self::$language['quantity']?></td>
                <td ><?php echo self::$language['has']?><?php echo self::$language['loss']?></td>
                <td ><?php echo self::$language['lately']?><?php echo self::$language['stocktake']?></td>
