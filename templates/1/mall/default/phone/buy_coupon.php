<div id=<?php echo $module['module_name'];?>  class="portlet light" monxin-module="<?php echo $module['module_name'];?>" align=left >
    ...
    function add(){
		...
		if(sum_quantityNew.prop('value')>10000){$("#<?php echo $module['module_name'];?> #state_new").html('<?php echo self::$language['must_be_less_than']?><10001');sum_quantityNew.focus();return false;}
		$("#<?php echo $module['module_name'];?> .add").css('display','none');
	    $("#<?php echo $module['module_name'];?> #state_new").html('<span class=\'fa fa-spinner fa-spin\'></span>');
        $.get('<?php echo $module['action_url'];?>&act=add',{name:nameNew.prop('value'),amount:amountNew.prop('value'),min_money:min_moneyNew.prop('value'),start_time:start_timeNew.prop('value'),end_time:end_timeNew.prop('value'),sum_quantity:sum_quantityNew.prop('value'),join_goods:$("#<?php echo $module['module_name'];?>  #join_goods_new").val()}, function(data){
            //alert(data);
			...
			$("#<?php echo $module['module_name'];?> .add").css('display','inline-block');
        });
    ...
    #<?php echo $module['module_name'];?>_html .number{width:80px;}
    #<?php echo $module['module_name'];?>_html .amount{width:80px;}
    #<?php echo $module['module_name'];?>_html .min_money{width:80px;}
    #<?php echo $module['module_name'];?>_html .start_time{width:100px;}
    #<?php echo $module['module_name'];?>_html .end_time{width:100px;}
