<div id=<?php echo $module['module_name'];?>  class="portlet light" monxin-module="<?php echo $module['module_name'];?>" align=left >
    ...
    $(document).ready(function(){
		...
		$('.slider_img').click(function(){
			...
			$("#monxin_iframe").attr('src','index.php?monxin=mall.replace_file&path='+replace_file+'&width=0&height=0&max_size=1024&min_size=1');
...
$.get('<?php echo $module['action_url'];?>&act=del',{id:id}, function(data){
	...
	function update_img_src(src){
		src=src.split('?');
		src=src[0];
		//alert(src);
		$("#<?php echo $module['module_name'];?>_html .slider_img a[file='"+src+"'] img").attr('src',src+'?re'.Math.random());
	}
    
    </script>
    <style>
    #<?php echo $module['module_name'];?> .id{ overflow:hidden; text-align:right;}
	.slider_img{ display:block; width:300px; overflow-x:scroll;}
	...
	#group_info{ font-size:24px; line-height:60px; font-weight:bold; border-bottom:#ccc  dashed 1px; margin-bottom:20px;}
    ...
    <div class="filter">
    	...
    	<span class=m_label><?php echo self::$language['image'];?></span><span id=img_temp></span><br />
        <span class=m_label><?php echo self::$language['name'];?></span><input id=name_new name=name_new /><br /> 
        <span class=m_label><?php echo self::$language['url'];?></span><input id=url_new name=url_new /> <br />
        <a href="#" onclick="return add()"  class='add'><?php echo self::$language['add']?></a> <span id=state_new  class='state'></span>
    </div>
