<div id=<?php echo $module['module_name'];?>  class="portlet light" monxin-module="<?php echo $module['module_name'];?>" align=left  >
    ...
    $(document).ready(function(){
		...
		$("#<?php echo $module['module_name'];?> .type").val($("#<?php echo $module['module_name'];?> .type").attr('monxin_value'));
		$("#<?php echo $module['module_name'];?> #banner").val('<?php echo $module['data']['banner']?>');
		$("#<?php echo $module['module_name'];?> #type_data").attr('src','./scanpay_type/'+$("#<?php echo $module['module_name'];?> .type").val()+'/config_panel.php');
	...
	function return_exits_data(){
		...
		data='<?php echo $module['data']['data']?>';
		if(data!=''){return eval('('+data+')');}else{return '';}
	}
	function submit_hidden(input_id){
		$("#<?php echo $module['module_name'];?> .banner_show").attr('src','./temp/'+$("#<?php echo $module['module_name'];?> #"+input_id).val());	
	}
    </script>
	...
	#<?php echo $module['module_name'];?> .line .m_label{ display: inline-block; vertical-align:top; width:32%; padding-right:5px; text-align:right;}
	#<?php echo $module['module_name'];?> .line .m_input{  display: inline-block; vertical-align:top; width:65%;}
	
	#<?php echo $module['module_name'];?> .line .m_input input{ width:65%;}
	#<?php echo $module['module_name'];?> .banner_show{ height:7rem;}
    </style>
    <div id="<?php echo $module['module_name'];?>_html">
        ...
        <div class=line><span class=m_label>&nbsp;</span><span class=m_input><img class=banner_show src="./program/scanpay/banner/<?php echo $module['data']['banner']?>" /></span></div>
        <div class=line><span class=m_label><?php echo self::$language['account_name']?>:</span><span class=m_input><input type=text class=name  placeholder="<?php echo self::$language['account_name_placeholder']?>" value="<?php echo $module['data']['name']?>" /> <span class=state></span></span></div>
        <div class=line><span class=m_label><?php echo self::$language['account_operator']?>:</span><span class=m_input><input type=text class=operator  placeholder="<?php echo self::$language['account_operator_placeholder']?>" value="<?php echo $module['data']['operator']?>"  /> <span class=state></span></span></div>
        <div class=line><span class=m_label><?php echo self::$language['account_type']?>:</span><span class=m_input><select class=type monxin_value="<?php echo $module['data']['type']?>" ><?php echo $module['type_option'];?></select> <span class=state></span></span></div>
