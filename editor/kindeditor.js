...
$(document).ready(function(){
	...
	$(document).on('click','.resize_img_switch',function(){open_resize_img();});
	$(document).on('click','.submit_resize_img',function(){
		 setTimeout("exe_resize_img()",1000);
		 return false;
	});
});

function exe_resize_img(){
	 img_min=parseFloat($(".resize_img .img_min").val());
	 img_width=$(".resize_img .img_width").val();
	 img_height=$(".resize_img .img_height").val();
	 $(".editor_content img").each(function(index, element) {
		if($(this).width()>img_min){
			$(this).css('width',img_width);
			$(this).css('height',img_height);
		}
	});
	 editor.html($(".editor_content").html());
}

function  open_resize_img(){
	$('.ie_warning .modal-title b').html('批量设置图片大小');
	$('.ie_warning .modal-body').html('<div class=resize_img>当图片宽度大于<input type=text class=img_min value="300" />像素，则宽度统一设为<input type=text class=img_width value="100%" />,高度<input type=text class=img_height value=auto /> <a class=submit_resize_img user_color=button>设置</a></div>');
	$('.ie_warning .modal-footer button').html('关闭');
	...
	$(".ie_warning").css('margin-top',($(window).height()-$(".ie_warning .modal-content").height())/8);	
	if(!$(".editor_content").html()){
		 $("body").append('<div class=editor_content>'+ editor.html()+'</div>'); 
	}else{
		 $(".editor_content").html( editor.html());
	}
	
}
	
function add_button(){
	///	alert($(".ke-toolbar .ke-outline").html());
	$(".ke-toolbar").append(' <span class=resize_img_switch>批调图片</span>');
}
	...
	_each(('cut,copy,paste').split(','), function(i, name) {
		self.clickToolbar(name, function() {
			...
			try {
				...
				monxin_alert(self.lang(name + 'Error'));
			}
		});
	});
