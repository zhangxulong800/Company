				...
				yesBtn : {
					...
					click : function(e) {
						...
						if (url == 'http://' || K.invalidUrl(url)) {
							monxin_alert(self.lang('invalidUrl'));
							urlBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(width)) {
							monxin_alert(self.lang('invalidWidth'));
							widthBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(height)) {
							monxin_alert(self.lang('invalidHeight'));
							heightBox[0].focus();
							return;
						}
				...
				var uploadbutton = K.uploadbutton({
					...
					afterUpload : function(data) {
						...
						if (data.error === 0) {
							...
							monxin_alert(self.lang('uploadSuccess'));
						} else {
							monxin_alert(data.message);
						}
					},
				...
				click : function(e) {
					...
					if (showLocal && showRemote && tabs && tabs.selectedIndex === 1 || !showRemote) {
						if (uploadbutton.fileBox.val() == '') {
							monxin_alert(self.lang('pleaseSelectFile'));
							return;
						}
						dialog.showLoading(self.lang('uploadLoading'));
						uploadbutton.submit();
...
KindEditor.plugin('map', function(K) {
	...
	self.clickToolbar(name, function() {
		...
		var iframeHtml = ['<!doctype html><html><head>',
			...
			'			});',
			'		} else {',
			'			monxin_alert("Invalid address: " + address);',
			'		}',
			'	});',
			'}',
			'</script>',
	...
	init : function(options) {
		...
		var settings = {
			...
			file_queue_error_handler : function(file, errorCode, message) {
				...
				K.DEBUG && monxin_alert(errorName);
			},
...
SWFUpload.Console.writeLine = function (message) {
	...
	try {
		...
		monxin_alert("Exception: " + ex.name + " Message: " + ex.message);
	}
};
				...
				yesBtn : {
					...
					click : function(e) {
						...
						if (rows == 0 || !/^\d+$/.test(rows)) {
							monxin_alert(self.lang('invalidRows'));
							rowsBox[0].focus();
							return;
						}
						...
						if (!/^\d*$/.test(padding)) {
							monxin_alert(self.lang('invalidPadding'));
							paddingBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(spacing)) {
							monxin_alert(self.lang('invalidSpacing'));
							spacingBox[0].focus();
							return;
						}
						if (!/^\d*$/.test(border)) {
							monxin_alert(self.lang('invalidBorder'));
							borderBox[0].focus();
							return;
						}
						//modify table
