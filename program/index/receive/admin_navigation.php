...
if($act=='add'){
	$sql="insert into ".$pdo->index_pre."navigation (`name`,`url`,`sequence`,`parent_id`,`open_target`,`visible`) values ('$name','$url','$sequence','$parent_id','$open_target','$visible')";
	if($pdo->exec($sql)){
		self::update_navigation($pdo);
...
if($act=='update'){
	$sql="update ".$pdo->index_pre."navigation set `parent_id`='$parent_id',`name`='$name',`url`='$url',`sequence`='$sequence',`open_target`='$open_target',`visible`='$visible' where `id`='$id'";
	if($pdo->exec($sql)){
		self::update_navigation($pdo);
...
if($act=='update_visible'){
	$sql="update ".$pdo->index_pre."navigation set `visible`='$visible' where `id`='$id'";	
	$pdo->exec($sql);
	self::update_navigation($pdo);
	exit();
}
if($act=='del'){
	...
	$sql="select count(id) as c from ".$pdo->index_pre."navigation where `parent_id`='$id'";
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['c']!=0){exit("{'state':'fail','info':'<span class=fail>&nbsp;</span>".self::$language['have_sub']."'}");}
	$sql="delete from ".$pdo->index_pre."navigation where `id`='$id'";
	if($pdo->exec($sql)){
		self::update_navigation($pdo);
		exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span>'}");
	}else{
...
if($act=='operation_visible'){
	...
	$ids=str_replace("|",",",$ids);
	$ids=trim($ids,",");
	$sql="update ".$pdo->index_pre."navigation set `visible`='$visible' where `id` in ($ids)";
	$pdo->exec($sql);
	self::update_navigation($pdo);
...
if($act=='submit_select'){
	...
	foreach($_POST as $v){
		...
		$sql="update ".$pdo->index_pre."navigation set `parent_id`='".$v['parent_id']."',`name`='".$v['name']."',`url`='".$v['url']."',`open_target`='".$v['open_target']."',`sequence`='".$v['sequence']."' where `id`='".$v['id']."'";
