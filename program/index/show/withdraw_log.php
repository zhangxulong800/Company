...
$sql="select * from ".$pdo->index_pre."withdraw where `username`='".$_SESSION['monxin']['username']."'";
...
foreach($r as $v){
	$v=de_safe_str($v);
	$pay_time='';
	if($v['state']!=3){$operation="<a href=".$v['id']." class='del'>".self::$language['del']."</a>";$operation="";}else{$operation="";}
	if($v['method']==0){$v['method']=self::$language['other'];}else{$v['method']=self::$language['openid'];}
	if($v['state']==2){$reason='<a href="#" title="'.$v['reason'].'" class="reason">'.self::$language['reason'].'</a>';}else{$reason='';$pay_time=get_time(self::$config['other']['date_style'],self::$config['other']['timeoffset'],self::$language,$v['pay_time']);}
...
$sql="select sum(money) as c from ".$pdo->index_pre."withdraw where `state`=3 and `username`='".$_SESSION['monxin']['username']."'";
if(@$_GET['method']!=''){$sql.=" and `method`='".safe_str($_GET['method'])."'";}
