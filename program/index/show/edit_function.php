<?php
		...
		$module['groups']=index::get_groups($pdo,0,"index.php?monxin=index.edit_function&program=".$program."&id=");
		$id=intval(@$_GET['id']);
		if($id==0){
			$sql="select `id` from ".$pdo->index_pre."group where `parent`=0 order by `sequence` desc limit 0,1";
			$r=$pdo->query($sql,2)->fetch(2);
			header("location:index.php?monxin=index.edit_function&id=".$r['id']);	
		}
