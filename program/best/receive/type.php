<?php
		...
		if($act=='add'){
			...
			if($_GET['parent']>0){
				$sql="select count(id) as c from ".self::$table_pre."page_type where `id`='".$_GET['parent']."'";
			...
			$sql="insert into ".self::$table_pre."page_type (`parent`,`name`,`sequence`,`visible`) values ('".$_GET['parent']."','".$_GET['name']."','".$_GET['sequence']."','".$_GET['visible']."')";
			if($pdo->exec($sql)){
		...
		if($act=='update'){
			...
			$_GET['visible']=intval(@$_GET['visible']);
			$_GET['name']=safe_str(@$_GET['name']);
			$sql="update ".self::$table_pre."page_type set `parent`='".$_GET['parent']."',`name`='".$_GET['name']."',`sequence`='".$_GET['sequence']."',`visible`='".$_GET['visible']."' where `id`='".$_GET['id']."'";
			if($pdo->exec($sql)){
		...
		if($act=='update_visible'){
			$_GET['id']=intval(@$_GET['id']);
			$_GET['visible']=intval(@$_GET['visible']);
			$sql="update ".self::$table_pre."page_type set `visible`='".$_GET['visible']."' where `id`='".$_GET['id']."'";	
			$pdo->exec($sql);
			exit();
		}
		if($act=='del'){
			$id=intval(@$_GET['id']);
			if($id<1){exit();}
			$sql="select count(id) as c from ".self::$table_pre."page_type where `parent`='$id'";
			$r=$pdo->query($sql,2)->fetch(2);
			if($r['c']!=0){exit("{'state':'fail','info':'<span class=fail>&nbsp;</span>".self::$language['have_sub']."'}");}
			
			$sql="select count(id) as c from ".self::$table_pre."page where `type`='$id'";
			$r=$pdo->query($sql,2)->fetch(2);
			if($r['c']!=0){exit("{'state':'fail','info':'<span class=fail>&nbsp;</span>".self::$language['have_sub']."'}");}
			
			$sql="delete from ".self::$table_pre."page_type where `id`='$id'";
			if($pdo->exec($sql)){
				exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span>'}");
			}else{
				exit("{'state':'fail','info':'<span class=fail>".self::$language['fail']."</span>'}");
		...
		if($act=='operation_visible'){
			...
			$ids=trim($temp,","); 
			$_GET['visible']=intval(@$_GET['visible']);
			$sql="update ".self::$table_pre."page_type set `visible`='".$_GET['visible']."' where `id` in ($ids)";
			$pdo->exec($sql);
		...
		if($act=='submit_select'){
			...
			foreach($_POST as $v){
				...
				$sql="update ".self::$table_pre."page_type set `parent`='".$v['parent']."',`name`='".$v['name']."',`sequence`='".$v['sequence']."' where `id`='".$v['id']."'";
