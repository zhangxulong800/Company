<?php
if(isset($_GET['auto_create_store'])){
	$id=intval(@$_GET['id']);
	$sql="select `username` from ".$pdo->index_pre."user where `id`=".$id;
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['username']){auto_create_store($pdo,$r['username']);}
	$id++;
	exit("<script>window.location.href='./index.php?monxin=bargain.master&auto_create_store=1&id=".$id."';</script>");
}
