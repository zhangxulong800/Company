...
if($goods_id>0){
	...
	if($r['id']==''){
		...
		if($pdo->exec($sql)){
			...
			echo '<script>alert("'.self::$language['success'].'");window.location.href="./index.php?monxin=bargain.goods_set&id='.$insret_id.'"</script>';
		}
	}
}
...
foreach($r as $v){
	...
	$pdo->exec($sql);
		...
		$list.="<tr id='tr_".$v['id']."'>
			...
			<td><a href=./index.php?monxin=bargain.goods_set&id=".$v['id'].">".self::$language['edit']."</a> <a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
		</tr>";
