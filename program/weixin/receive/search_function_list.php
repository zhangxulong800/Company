...
if($act=='add'){
	...
	$sql="select count(id) as c from ".self::$table_pre."search_function where `name`='".$name."'";
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['c']!=0){exit("{'state':'fail','info':'<span class=fail>".self::$language['exist_same'].self::$language['function_name']."</span>'}");}
	$path='./program/weixin/search_function/'.$name.'.php';
	if(!file_exists($path)){exit("{'state':'fail','info':'<span class=fail>".$path.' '.self::$language['not_exist'].self::$language['please_upload']."</span>'}");}
	
	$sql="insert into ".self::$table_pre."search_function (`name`,`description`,`sequence`,`state`) values ('".$_GET['name']."','".$_GET['description']."','".$_GET['sequence']."','".$_GET['state']."')";
	if($pdo->exec($sql)){
...
if($act=='update'){
	...
	$path='./program/weixin/search_function/'.$_GET['name'].'.php';
	if(!file_exists($path)){exit("{'state':'fail','info':'<span class=fail>".$path.' '.self::$language['not_exist'].self::$language['please_upload']."</span>'}");}
	$sql="select count(id) as c from ".self::$table_pre."search_function where `name`='".$_GET['name']."' and `id`!=".$_GET['id'];
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['c']!=0){exit("{'state':'fail','info':'<span class=fail>".self::$language['exist_same'].self::$language['function_name']."</span>'}");}
	$sql="update ".self::$table_pre."search_function set `name`='".$_GET['name']."',`description`='".$_GET['description']."',`sequence`='".$_GET['sequence']."',`state`='".$_GET['state']."' where `id`='".$_GET['id']."'";
	if($pdo->exec($sql)){
...
if($act=='del'){
	$id=intval(@$_GET['id']);		
	$sql="delete from ".self::$table_pre."search_function where `id`='$id'";
	if($pdo->exec($sql)){
		exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span>'}");
	}else{
		exit("{'state':'fail','info':'<span class=fail>".self::$language['fail']."</span>'}");
...
if($act=='submit_select'){
	...
	foreach($_POST as $v){
		...
		$path='./program/weixin/search_function/'.$v['name'].'.php';
		if(!file_exists($path)){continue;}
		$sql="select count(id) as c from ".self::$table_pre."search_function where `name`='".$v['name']."' and `id`!=".$v['id'];
		$r=$pdo->query($sql,2)->fetch(2);
		if($r['c']!=0){continue;}
		$sql="update ".self::$table_pre."search_function set `name`='".$v['name']."',`description`='".$v['description']."',`sequence`='".$v['sequence']."',`state`='".$v['state']."' where `id`='".$v['id']."'";
