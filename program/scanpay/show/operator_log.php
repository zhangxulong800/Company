...
$sql="select `id`,`name`,`type` from ".self::$table_pre."account where (`username`='".$_SESSION['monxin']['username']."' or `operator` like '%".$_SESSION['monxin']['username'].",%')";
$r=$pdo->query($sql,2);
$account=array();
$account_type=array();
$account_option='';
...
$sql="select * from ".self::$table_pre."pay where `operator`='".$_SESSION['monxin']['username']."' ";
...
$list='';
foreach($r as $v){
	if(!isset($account[$v['a_id']])){continue;}
	$inquiry='';
	...
	<td >".$v['money']."</td>
	<td >".self::$language['pay_state'][$v['state']]." ".$inquiry."</td>
...
$sql="select  sum(`money`) as c from ".self::$table_pre."pay where `operator`='".$_SESSION['monxin']['username']."' ".$where;
$sql=str_replace("_pay and","_pay where",$sql);
