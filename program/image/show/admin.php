<?php
if(@$_GET['act']=='link'){
	$sql="select `link`,`id` from ".self::$table_pre."img where `link`!=''";
	$r=$pdo->query($sql);
	foreach($r as $v){
		$link=str_replace('192.168.28.212/fxzl/','www.monxin.com/',$v['link']);
		$sql="update ".self::$table_pre."img set `link`='".$link."' where `id`=".$v['id'];
		$pdo->exec($sql);
	}	
...
foreach($r as $v){
	...
	<td><input type='text' name='title_".$v['id']."' id='title_".$v['id']."' value='".$v['title']."'  class='title' /><br /><a href='index.php?monxin=".$class.".show&id=".$v['id']."&type=".$_GET['type']."&search=".urlencode($_GET['search'])."&order=".$_GET['order']."' target=_blank><img src='./program/image/img_thumb/".$v['src']."' class=thumb_img /></a></td>
  ...
  <td class=operation_td><a href='index.php?monxin=".$class.".show&id=".$v['id']."&type=".$_GET['type']."&search=".urlencode($_GET['search'])."&order=".$_GET['order']."' target=_blank  class='view'>".self::$language['view']."</a> <a href='#' onclick='return update(".$v['id'].")'  class='submit'>".self::$language['submit']."</a> <a href='index.php?monxin=".$class.".edit&id=".$v['id']."' target=_blank class='edit'>".self::$language['edit']."</a> <a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
</tr>
";	
}
