<?php
/**
 *	商城数据接收类 示例 ./receive.php?target=gbuy::goods_add (target=类名::方法名),大部分情况是通过 __call方法 加载执行 ./program/gbuy/receive/ 目录下的对应名称的文件。
 */
class receive{
...
//======================================================================================================= 获取 店铺ID
	...
	function delete_gbuy($pdo,$id){
		...
		if($pdo->exec($sql)){
			$sql="delete from ".self::$table_pre."group where `b_id`=".$id;
			$pdo->exec($sql);
			return true;
		}
		return false;
	...
	function delete_gbuy_log($pdo,$id){
