...
$sql="select * from ".self::$table_pre."order where `username`='".$_SESSION['monxin']['username']."' and `buyer_delete`=0";
$where='';

if(isset($_GET['state'])){
	$where.=" and `state`=".intval($_GET['state']);
}
if($_GET['search']!=''){$where=" and (`g_title` like '%".$_GET['search']."%' or `out_id` ='".$_GET['search']."')";}
...
foreach($r as $v){
	...
	$act='';
	if($v['state']==0){$act="<a href=./index.php?monxin=gbuy.pay&id=".$v['id']." >".self::$language['payment'].self::$language['orders']."</a>";}
	if($v['state']!=1){$act.="<a  onclick='return del(".$v['id'].")'  class='del'>".self::$language['del'].self::$language['orders']."</a>";}
	$list.="<div class=border id=tr_".$v['id']."><div class=o_head><span class=h_left>".get_date($v['time'],self::$config['other']['date_style'],self::$config['other']['timeoffset'])." <span class=out_id>".$v['out_id']."</span></span><span class=h_right>".self::$language['order_state_option'][$v['state']]."</span></div><div class=goods>
...
$module['state_option']='<a href=./index.php?monxin=gbuy.my_buy class=current>'.self::$language['all'].'</a>';
foreach(self::$language['order_state_option'] as $k=>$v){
	$module['state_option'].='<a href=./index.php?monxin=gbuy.my_buy&state='.$k.' state='.$k.'>'.$v.'</a>';
}
