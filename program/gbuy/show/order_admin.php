...
$sql="select * from ".self::$table_pre."log where `state`=4";
$where='';
...
foreach($r as $v){
	...
	$pdo->exec($sql);
		...
		$list.="<tr id='tr_".$v['id']."'>
			...
			<td><a href='./index.php?monxin=gbuy.detail&id=".$v['b_id']."&l=".$v['id']."' target=_blank class='icon'><img src='./program/mall/img_thumb/".$g['icon']."'></a></td>
			<td><div class=title><a href='./index.php?monxin=gbuy.detail&id=".$v['b_id']."&l=".$v['id']."' target=_blank>".$g['title']."</a></div></td>
			<td>".$v['username']."</td>
			<td>".$v['quantity']."</td>
			<td><a href=./index.php?monxin=gbuy.log_detail&id=".$v['id']." class=log_detail>".$v['money']."</a></td>
			<td><a href=./index.php?monxin=mall.m_order_admin&id=".$v['order_id']." target=_blank class=go_order>".$v['order_money']."</a></td>
			
			
			<td><a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
		</tr>";
