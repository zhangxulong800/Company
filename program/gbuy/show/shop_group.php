...
foreach($r as $v){
	...
	$list.="<div class=border id=tr_".$v['id']."><div class=o_head><span class=h_left><span class=g_username>".self::$language['group_founder'].": ".$v['username']."</span><span class=start>".self::$language['open_gbuy_time'].': '.get_date($v['start'],self::$config['other']['date_style'].' H:i',self::$config['other']['timeoffset'])."</span><span class=time_limit remain=".(($v['start']+($goods_hour[$v['g_id']]['hour']*3600))-time()).">".self::$language['']."</span><span class=g_earn>".self::$language['g_earn'].": ".self::$language['money_symbol'].$v['earn']."</span></span><span class=h_right>".self::$language['group_state_option'][$v['state']]."</span></div><div class=goods>
	<div class=g_left><img src='./program/mall/img_thumb/".$goods_info[$v['g_id']]['icon']."' /></div><div class=g_right>
		...
		<div class=group_list>".self::get_gbuy_group_show($pdo,$v['id'])." <a class=view_order href=./index.php?monxin=gbuy.shop_order&group=".$v['id'].">".self::$language['view_order']."</a></div>
	</div>
	</div>
...
$module['state_option']='<a href=./index.php?monxin=gbuy.shop_group class=current>'.self::$language['all'].'</a>';
foreach(self::$language['group_state_option'] as $k=>$v){
	$module['state_option'].='<a href=./index.php?monxin=gbuy.shop_group&state='.$k.' state='.$k.'>'.$v.'</a>';
}
