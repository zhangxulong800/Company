...
$id=intval(@$_GET['id']);
if($id>0){
	$sql="select * from ".self::$table_pre."shop_type where `id`='$id' and `shop_id`=".SHOP_ID;
	$r=$pdo->query($sql,2)->fetch(2);
	$time=time();
	$_POST['content']=safe_str(@$_POST['content']);
	
	$sql="update ".self::$table_pre."shop_type set `remark`='".$_POST['content']."' where `id`='$id' and `shop_id`=".SHOP_ID;
	if($pdo->exec($sql)){
		...
		foreach($old_imgs as $v){
			if(!in_array($v,$new_imgs)){
				$sql="select count(id) as c from ".self::$table_pre."shop_type where `remark` like '%".$v."%'";
				$r=$pdo->query($sql,2)->fetch(2);
				if($r['c']==0){
					$path=$v;
					safe_unlink($path);
