...
if($act=='update'){
	...
	$sql="update ".self::$table_pre."module set `width`='".$_GET['width']."',`height`='".$_GET['height']."',`title`='".$_GET['title']."',`sequence`='".$_GET['sequence']."',`time`='$time' where `id`='".$_GET['id']."' and `shop_id`=".SHOP_ID;
	if($pdo->exec($sql)){
...
if($act=='del'){
	...
	$sql="delete from ".self::$table_pre."module where `id`='$id' and `shop_id`=".SHOP_ID;
	if($pdo->exec($sql)){
		...
		foreach($fields as $field){
			$sql="select `$field`,`id` from ".self::$table_pre."page where `$field` like '%mall.diymodule_show_".$id."%'";
			$r=$pdo->query($sql,2);
			
			foreach($r as $v){
				$new_field=str_replace("mall.diymodule_show_".$id.",","",$v[$field]);
				$new_field=str_replace("mall.diymodule_show_".$id."","",$new_field);
...
if($act=='del_select'){
	...
	foreach($ids as $id){
		$id=intval($id);
		$sql="select `content` from ".self::$table_pre."module where `id`='$id' and `shop_id`=".SHOP_ID;
		$r=$pdo->query($sql,2)->fetch(2);
		$sql="delete from ".self::$table_pre."module where `id`='$id' and `shop_id`=".SHOP_ID;
		if($pdo->exec($sql)){
			$this->update_module($pdo,$id);	
...
if($act=='submit_select'){
	...
	foreach($_POST as $v){
		...
		$sql="update ".self::$table_pre."module set `width`='".$v['width']."',`height`='".$v['height']."',`title`='".$v['title']."',`sequence`='".$v['sequence']."',`time`='$time' where `id`='".$v['id']."' and `shop_id`=".SHOP_ID;
