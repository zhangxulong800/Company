...
if($act=='submit_checkout'){
	...
	if($_POST['user']!=''){
			...
			if($pre=='6666_' && strlen($postfix)==8){
				...
				if($qr_info['id']!=''){
					if($qr_info['username']!=$buyer['username']){
						exit('{"state":"fail","info":"<span class=fail>'.self::$language['only_self_pay_code'].'</span>"}');
					}
					$buyer['qr_pay']=$qr_info['id'];
				}else{
					exit('{"state":"fail","info":"<span class=fail>'.self::$language['pay_code_err'].'</span>"}');
				}
			}else{
		...
		if(intval(@$_POST['checkout_id'])!=0 && $pay_method!='cash' && $pay_method!='weixin_p' && $pay_method!='alipay_p' && @$_POST['transaction_password']==''){
			$need_password=1;
		...
		if($pay_method=='balance'){
			if($buyer['money']<$payable){
				...
				//if($seller_u['money']<$remaining){exit('{"state":"fail","info":"<span class=fail>'.self::$language['cashier'].self::$language['insufficient_balance'].'</span>"}');}
	...
	if($pay_method=='cash' && is_numeric($_POST['received_money'])){
		...
		if($received_money<$payable && !monxin_number_difference($received_money,$payable,0.3)){exit('{"state":"fail","info":"<span class=fail>received_money err</span>"}');}
