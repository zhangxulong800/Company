...
$act=@$_GET['act'];
if($act=='add_exist_id'){
	$sql="select `id`,`username` from ".$pdo->index_pre."user where `username`='".safe_str($_POST['username'])."' and `license_id`='".safe_str($_POST['license_id'])."' limit 0,1";
	$r=$pdo->query($sql,2)->fetch(2);
	if($r['username']!=''){
		self::add_shop_buyer($pdo,$r['username'],SHOP_ID);
		exit("{'state':'success','info':'<span class=success>".self::$language['success']." , <a href=index.php?monxin=mall.my_buyer>".self::$language['view']."</a></span>'}");
	}else{
