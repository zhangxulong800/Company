...
if($act=='inquiry'){
	...
	$sql="select `reason` from ".self::$table_pre."goods_loss where `goods_id`=".$r['id']." group by `reason` order by `id` desc limit 0,10";
	$rr=$pdo->query($sql,2);
	$i=0;
	foreach($rr as $v){
	...
	if($i<10){
		$sql="select `reason` from ".self::$table_pre."goods_loss where `goods_id`!=".$r['id']." group by `reason` order by `id` desc limit 0,".(10-$i);
		$rr=$pdo->query($sql,2);
		foreach($rr as $v){
			$v['reason']=de_safe_str($v['reason']);
			$r['loss_reason_option'].='<option value="'.$v['reason'].'">'.$v['reason'].'</option>';
...
if($act=='add'){
	...
	if($pdo->exec($sql)){
		...
		$sql="insert into ".self::$table_pre."goods_loss (`goods_id`,`s_id`,`quantity`,`money`,`reason`,`time`,`username`,`shop_id`) values ('".$id."','".$option_id."','".$quantity."','".(self::get_cost_price($pdo,$goods_id)*$quantity)."','".$reason."','".$time."','".$_SESSION['monxin']['username']."','".SHOP_ID."')";
		if($pdo->exec($sql)){
