...
if($act=='add'){
	...
	$sql="insert into ".self::$table_pre."slider_img (`name`,`url`,`group_id`) values ('".$_GET['name']."','".$_GET['url']."','".$_GET['group_id']."')";
	//echo $sql;
	if($pdo->exec($sql)){
		//echo $_GET['img'];
		if(file_exists('./temp/'.$_GET['img'])){safe_rename('./temp/'.$_GET['img'],'./program/mall/slider_img/'.$pdo->lastInsertId().'.jpg');}
...
if($act=='update'){
	...
	$sql="update ".self::$table_pre."slider_img set `name`='".$_GET['name']."',`url`='".$_GET['url']."',`target`='".$_GET['target']."',`sequence`='".$_GET['sequence']."' where `id`='".$_GET['id']."'";
	if($pdo->exec($sql)){
		update_table_group($pdo,$group_id);
...
if($act=='del'){
	$sql="delete from ".self::$table_pre."slider_img where `id`='$id'";
	if($pdo->exec($sql)){
		update_table_group($pdo,$group_id);
		@safe_unlink('./program/mall/slider_img/'.$id.'.jpg');
		exit("{'state':'success','info':'<span class=success>".self::$language['success']."</span>'}");
	}else{
		exit("{'state':'fail','info':'<span class=fail>".self::$language['fail']."</span>'}");
	}
...
if($act=='submit_select'){
	...
	foreach($_POST as $v){
		...
		$sql="update ".self::$table_pre."slider_img set `name`='".$v['name']."',`url`='".$v['url']."',`target`='".$v['target']."',`sequence`='".$v['sequence']."' where `id`='".$v['id']."'";
