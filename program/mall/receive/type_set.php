...
if($act=='save'){
	...
	$sql="update ".self::$table_pre."goods set `type`='".$type."' where `id`=".$_GET['c_id'];	
	if($pdo->exec($sql)){
		$html=str_replace('/a>','/a> >',self::get_type_position($pdo,$type)).' <a href=./index.php?monxin=mall.type_set&c_id='.$_GET['c_id'].'&id='.$type.' class=set>&nbsp;</a>';
