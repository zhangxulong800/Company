...
if($act=='submit_checkout'){
	if($_POST['user']!=''){
		...
		$where='';
		if(preg_match(self::$config['other']['reg_phone'],$username)){$where="`phone`='".$username."'";}
		if($where==''){
		...
		$sql="select `id`,`real_name`,`nickname`,`money`,`state`,`username`,`transaction_password` from ".$pdo->index_pre."user where ".$where." limit 0,1";
		$r=$pdo->query($sql,2)->fetch(2);
		if($r['id']==''){
			$sql="select `id`,`real_name`,`nickname`,`money`,`state`,`username`,`transaction_password` from ".$pdo->index_pre."user where `chip`='".$username."' limit 0,1";
			$r=$pdo->query($sql,2)->fetch(2);
		}
		...
		$sql="select `group_id`,`balance` from ".self::$table_pre."shop_buyer where `shop_id`=".SHOP_ID." and `username`='".$buyer['username']."' limit 0,1";
		$r=$pdo->query($sql,2)->fetch(2);
	...
	if($pay_method!='cash' && $pay_method!='pos' && $pay_method!='credit' && $pay_method!='balance' && $pay_method!='shop_balance' && $pay_method!='weixin' && $pay_method!='alipay'){exit('{"state":"fail","info":"<span class=fail>pay_method err</span>"}');}
	...
	foreach($checkout_desk as $key=>$v){
		...
		$order_goods_sql[]="insert into ".self::$table_pre."order_goods (`goods_id`,`s_id`,`shop_id`,`order_id`,`snapshot_id`,`icon`,`title`,`quantity`,`price`,`transaction_price`,`unit`,`time`,`buyer`,`order_state`) values ('".$r['id']."','".$s_id."','".SHOP_ID."','{order_id}','".$snapshot_id."','".$icon."','".$r['title']."','".$checkout_desk[$key]['quantity']."','".$price."','".$price."','".self::get_mall_unit_name($pdo,$r['unit'])."','".$time."','".@$buyer['username']."','6')";
		$goods_names.=$r['title']."
		";;
	...
	if($pay_method=='alipay' || $pay_method=='weixin'){
		...
		if($pay['money']!=$payable){exit('{"state":"fail","info":"<span class=fail>scanpay money err</span>"}');}
	...
	if($_POST['user']!=''){
		...
		if($pay_method=='balance'){
			if($buyer['money']<$payable){exit('{"state":"fail","info":"<span class=fail>'.self::$language['insufficient_balance'].'</span>"}');}
		}
		if($pay_method=='shop_balance'){
			if($buyer['shop_balance']<$payable){exit('{"state":"fail","info":"<span class=fail>'.self::$language['insufficient_balance'].'</span>"}');}
		}
		
		if($pay_method=='shop_balance' || $pay_method=='balance'){
	...
	if($pay_method=='cash' && is_numeric($_POST['received_money'])){
		...
		if($received_money<$payable){exit('{"state":"fail","info":"<span class=fail>received_money err</span>"}');}
	...
	if(!$pdo->exec($sql)){
		...
		if($pay_method=='balance'){
			...
			$reason=str_replace('{sum_money}',$payable,$reason);
			if(!operator_money(self::$config,self::$language,$pdo,$buyer['username'],'-'.$payable,$reason,'mall')){
				$pdo->exec("delete from ".self::$table_pre."order where `id`=".$order_id);
		...
		if($pay_method=='shop_balance'){
			...
			$reason=str_replace('{order_id}',$order['out_id'],self::$language['deduction_order_money_template']);
			$reason=str_replace('{sum_money}',$payable,$reason);
			if(!self::operator_shop_buyer_balance($pdo,$buyer['username'],'-'.$payable,$reason)){
				$pdo->exec("delete from ".self::$table_pre."order where `id`=".$order_id);
				exit('{"state":"fail","info":"<span class=fail>decrease money err</span>"}');
			}
		...
		if(intval(@$_COOKIE['checkout_print_set'])==1){//组织打印小票内容 
			...
			$sql="select `title`,`transaction_price`,`quantity`,`unit`,`s_id`,`goods_id`,`price` from ".self::$table_pre."order_goods where `order_id`=".$order['id'];
			$r2=$pdo->query($sql,2);
			$list2='';
			foreach($r2 as $v2){		
				...
				$list2.='<div class=goods>'.$v2['title'].' <span>'.str_replace('.00','',$v2['price']).'*'.self::format_quantity($v2['quantity']).$v2['unit'].'='.str_replace('.00','',$v2['price']*$v2['quantity']).'</span></div>';	
		...
		<div class=order>
            ...
            <div class=goods_info>'.$list2.'</div>
			'.$preferential_way.'
            <div class=actual_money>'.self::$language['actual_pay'].str_replace('.00','',$order['actual_money']).self::$language['yuan'].' '.$order['change_price_reason'].'</div>
...
';	
		...
		if(intval(@$_COOKIE['checkout_print_set'])==2){
			...
			$module['list']='';
			foreach($r as $v){
				$sql="select `title`,`transaction_price`,`quantity`,`unit`,`s_id`,`goods_id`,`price` from ".self::$table_pre."order_goods where `order_id`=".$v['id'];
				$r2=$pdo->query($sql,2);
				$list2='';
				foreach($r2 as $v2){		
					$list2.="<div class=goods><span class=goods_id>".$v2['goods_id']."</span><span class=goods_name>".$v2['title']."</span><span class=goods_price>".$v2['price']."</span><span class=goods_quantity>".self::format_quantity($v2['quantity']).$v2['unit']."</span><span class=goods_money>".$v2['price']*$v2['quantity']."</span></div>";	
				}
				$sql="select `name` from ".self::$table_pre."shop where `id`=".SHOP_ID;
				$r=$pdo->query($sql,2)->fetch(2);
				$shop_name=$r['name'];
				...
				if($v['goods_money']+$v['express_cost_buyer']-$v['sum_money']!=0){
					$preferential_way="<div class=normal_price>".self::$language['original_price'].self::$language['sum'].":".$v['goods_money'].self::$language['yuan']."</div><div class=preferential_way>".$v['preferential_code'].self::$language['preferential_way_option'][$v['preferential_way']].": -". sprintf('%.2f',$v['goods_money']+$v['express_cost_buyer']-$v['sum_money']).self::$language['yuan']."</div>";				
				}else{$preferential_way='';}
				...
				$module['list'].="<div class=order_div>
						...
						<div class=order_body>
							<div class='goods thead'><span class=goods_id>".self::$language['goods'].self::$language['number']."</span><span class=goods_name>".self::$language['goods'].self::$language['name']."</span><span class=goods_price>".self::$language['price']."</span><span class=goods_quantity>".self::$language['quantity']."</span><span class=goods_money>".self::$language['money']."</span></div>
							".$list2."
						</div>
						<div class=order_foot>".$preferential_way.self::$language['actual_pay'].str_replace('.00','',$v['actual_money']).self::$language['yuan'].'('.self::$language['freight_costs'].str_replace('.00','',$v['express_cost_buyer']).self::$language['yuan'].') '.$v['change_price_reason']."</div>
					</div>";
...
if($act=='submit'){
	...
	if(is_numeric($barcode)){
		...
		if(($r['bar_code']==$barcode || $r['store_code']==$barcode) && $r['option_enable']==0 ){
			...
			$g[$key]['title']=$r['title'];	
			$spec_info=self::get_type_option_name($pdo,$r2['option_id']).' '.$r2['color_name'];
	...
	<td class=goods_td'.$have_spec.'><a href=./index.php?monxin=mall.goods&id='.$r['id'].' class=goods_info target=_blank><span class=title>'.$g[$key]['title'].'</span><span class=spec>'.$spec_info.'</span></a></td>
	...
	<td class=operation_td><span class=state></span></td>
	</tr>';
