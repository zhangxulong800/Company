...
$sql="select `id`,`out_id`,`actual_money`,`cashier`,`add_time`,`pay_method`,`preferential_way`,`goods_money`,`buyer`,`reference_number` from ".self::$table_pre."order where `shop_id`=".SHOP_ID." and `state`=6";
...
$where=" and `cashier`='".$_SESSION['monxin']['username']."'";
...
foreach($r as $v){
	...
	$list.='<tr id=tr_'.$v['id'].'><td class=id>'.$v['out_id'].'</td><td class=buyer>'.$v['buyer'].'</td><td class=actual_money>'.$v['actual_money'].'</td><td class=pay_method>'.self::$language['pay_method'][$v['pay_method']].$v['reference_number'].'</td><td class=preferential_way>'.self::$language['preferential_way_option'][$v['preferential_way']].'-'.$d.'</td><td class=time>'.get_time(self::$config['other']['date_style'],self::$config['other']['timeoffset'],self::$language,$v['add_time']).'</td><td class=operation_td><a href=./index.php?monxin=mall.checkout_detail&id='.$v['id'].' target=_blank>'.self::$language['detail'].'</a> <a href=./index.php?monxin=mall.order_print_list&ids='.$v['id'].' target=_blank>'.self::$language['print_set'][2].'</a> <a href=./index.php?monxin=mall.order_print&ids='.$v['id'].' target=_blank>'.self::$language['print_set'][1].'</a> <a class=yun_print>'.self::$language['yun_print'].'</a> <span id=state_'.$v['id'].'></span></td></tr>';	
}
if($sum==0){
...
$sql="select sum(`actual_money`) as c ,sum(`web_credits_money`) as c2 from ".self::$table_pre."order".$where." and `state`=6";
$sql=str_replace("_order and","_order where",$sql);
$sql=str_replace("_order  and","_order where",$sql);
$r=$pdo->query($sql,2)->fetch(2);
$module['sum']['sum']=floatval($r['c'])-floatval($r['c2']);
