...
$sql="select * from ".self::$table_pre."shop_buyer_credits where `username`='".$r['username']."' and `shop_id`=".SHOP_ID;
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	...
	$sum_sql=str_replace("_shop_buyer_credits and","_shop_buyer_credits where",$sum_sql);
	$r=$pdo->query($sum_sql,2)->fetch(2);
	$sum=$r['c'];
$sql=$sql.$where.$order.$limit;
$sql=str_replace("_shop_buyer_credits and","_shop_buyer_credits where",$sql);
//echo($sql);
//exit();
$r=$pdo->query($sql,2);
$list='';
