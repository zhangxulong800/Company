...
if($r['state']!=1){exit("<script>alert('".$g_l['goods_state_option'][$r['state']]."');window.location.href='./index.php?monxin=gbuy.list';</script>");}
if($r['start_time']>time()){exit("<script>alert('".$g_l['gbuy_no_start']."');window.location.href='./index.php?monxin=gbuy.list';</script>");}
if($r['end_time']+$r['hour']*3600<time()){exit("<script>alert('".$g_l['gbuy_end']."');window.location.href='./index.php?monxin=gbuy.list';</script>");}
...
if(isset($_GET['group'])){
	...
	if($group>0){
		...
		$sql="select `id`,`state` from ".$pdo->sys_pre."gbuy_order where `gr_id`=".$group." and `username`='".$_SESSION['monxin']['username']."' limit 0,1";
		$o=$pdo->query($sql,2)->fetch(2);
		if($o['id']!=''){
			if($o['state']==0){
				exit("<script>alert('".$g_l['gbuy_joined']."');window.location.href='./index.php?monxin=gbuy.pay&id=".$o['id']."';</script>");
			}else{
				exit("<script>alert('".$g_l['gbuy_joined']."');window.location.href='./index.php?monxin=gbuy.detail&group=".$group."';</script>");
			}
		}
...
$temp=explode('mall.gbuy_confirm_order',$module['current_url']);
$_SESSION['token'][$method]=get_random(8);$module['action_url']="receive.php?token=".$_SESSION['token'][$method]."&target=".$method.'&goods_src='.$_GET['goods_src'].'&group='.@$_GET['group'].'&'.$temp[1];
...
$module['receiver'].='<a href="#" class="option" id=receiver_-1 style="display:inline-block;vertical-align:top;" top_id="0">
                        ...
                        <div class=phone>'.self::$language['take_self'].'</div>
                    </a>';
...
if($goods_src=='goods_id'){//▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃▃ 如是单项购买
	...
	//if(isset($_GET['gid'])){
		...
		$sql="select * from ".$pdo->sys_pre."gbuy_goods where `g_id`=".$id;
		//echo $sql;
		$gbuy=$pdo->query($sql,2)->fetch(2);
		if($gbuy['id']!=''){
			$module['gbuy']=$gbuy['price'];
		}
		//var_dump($module['gbuy']);
	//}
	//=============================================================================================================拼团 end
	...
	if(!isset($shops[$goods[$v]['shop_id']]['index'])){
		...
		foreach($r as $e){
			...
			foreach($r2 as $e2){
				...
				$prices[$e2['area_id']]['f_p']=$e2['first_price'];	
				$prices[$e2['area_id']]['c_p']=$e2['continue_price'];	
			}
...
foreach($shops as $k=>$v){
	if($pre_sale){
		...
		if($full_pre==='free_shipping'){$full_pre=self::$language['free_shipping'];}else{$full_pre='-'.$full_pre;}
	}
