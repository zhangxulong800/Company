...
$sql="select `id`,`type`,`title`,`icon`,`sold`,`position`,`inventory`,`unit`,`supplier`,`visit`,`time`,`shop_sequence`,`state`,`shop_tag`,`shop_type`,`option_enable`,`bidding_show`,`shop_id` from ".self::$table_pre."goods where `share`=1";
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	$sum_sql=$sql.$where;
	$sum_sql=str_replace(" `id`,`type`,`title`,`icon`,`sold`,`position`,`inventory`,`unit`,`supplier`,`visit`,`time`,`shop_sequence`,`state`,`shop_tag`,`shop_type`,`option_enable`,`bidding_show`,`shop_id` "," count(id) as c ",$sum_sql);
...
foreach($r as $v){
	...
	<td><div class=title><a href='./index.php?monxin=mall.goods&id=".$v['id']."' target=_blank>".$v['title']."</a></div><div class=type_tag><span class=type>".self::$language['mall'].self::$language['type'].": ".self::get_type_position($pdo,$v['type'])." ".$edit_type."</span><span class=shop_type>".self::$language['headquarters'].": ".self::get_shop_name($pdo,$v['shop_id'])."</span></div>
	</td>
