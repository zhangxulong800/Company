...
$sql="select `goods_id`,`time` from ".self::$table_pre."visit where `username`='".$_SESSION['monxin']['username']."' order by `id` desc limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	$sum_sql="select count(id) as c  from ".self::$table_pre."visit where `username`='".$_SESSION['monxin']['username']."'";
	//echo $sum_sql;
...
$list='';
foreach($index as $v){
	if(isset($goods[$v])){$list.=$goods[$v];}else{
		$sql="delete from ".self::$table_pre."visit where `goods_id`=".$v;
		$pdo->exec($sql);	
	}
