...
// check
if($dir!='./program/mall/slider_img/'){
	echo self::$language['act_noPower'];return false;	
}
if(intval($file_name)==0){echo self::$language['act_noPower'];return false;	}
$sql="select `group_id` from ".self::$table_pre."slider_img where `id`=".intval($file_name);
$r=$pdo->query($sql,2)->fetch(2);
if($r['group_id']==''){echo self::$language['act_noPower'];return false;}
$sql="select `shop_id` from ".self::$table_pre."slider where `id`=".$r['group_id'];
$r=$pdo->query($sql,2)->fetch(2);
