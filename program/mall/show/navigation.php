...
function admin_navigation_get_parent($pdo,$v,$id){
	$sql="select `name`,`id` from ".$pdo->sys_pre."mall_navigation where `shop_id`=".SHOP_ID." and `visible`='1' and `parent_id`=0 and `id`!='$id' order by `sequence` desc";
	...
	foreach($stmt as $v){
		...
		$sql2="select `name`,`id` from ".$pdo->sys_pre."mall_navigation where `shop_id`=".SHOP_ID." and `visible`='1' and `parent_id`=".$v['id']." and `id`!='$id' order by `sequence` desc";
		$r=$pdo->query($sql2,2);
		foreach($r as $v2){
...
function admin_navigation_get_list($pdo,$language,$parent_id,$module_name){
	...
	$search=trim($search);
	$sql="select * from ".$pdo->sys_pre."mall_navigation";
	...
	foreach($r as $v){
		...
		<td><input type='text' name='name_".$v['id']."' id='name_".$v['id']."' value='".$v['name']."'  class='name' style='width:".admin_navigation_get_name_width($pdo,$v['id'])."%;' /><a href=# id=icon_".$v['id']." class=icon><img src='./program/index/navigation_icon/".$v['id'].".png'></a></td>
...
";	
	$sql2="select count(id) as c from ".$pdo->sys_pre."mall_navigation where `shop_id`=".SHOP_ID." and `parent_id`=".$v['id']."";
...
function admin_navigation_get_name_width($pdo,$id){
	...
	$sql="select `parent_id`,`id` from ".$pdo->sys_pre."mall_navigation where `shop_id`=".SHOP_ID." and `id`='$id'";
	$v=$pdo->query($sql,2)->fetch(2);
	if($v['parent_id']==0){return 80;}	
	$sql="select `parent_id`,`id` from ".$pdo->sys_pre."mall_navigation where `shop_id`=".SHOP_ID." and `id`='".$v['parent_id']."'";
	$v=$pdo->query($sql,2)->fetch(2);
