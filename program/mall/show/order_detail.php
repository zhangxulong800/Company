...
$_SESSION['token']['mall::order_admin']=get_random(8);$module['action_url']="receive.php?token=".$_SESSION['token']['mall::order_admin']."&target=mall::order_admin";
	...
	foreach($r2 as $v2){
		...
		if($v['preferential_way']==5){$o_price='<span class=o_price>'.$v2['price'].'</span>';$v2['price']=$v2['transaction_price'];}else{$o_price='';}
		  ...
		  <span class=icon>
			  ...
			  <div class=price>".self::$language['price'].":".$o_price.' '.str_replace('.00','',$v2['price']).self::$language['yuan']." &nbsp; ".self::$language['quantity'].":".self::format_quantity($v2['quantity']).$v2['unit']." <a href=./index.php?monxin=mall.goods_snapshot&id=".$v2['snapshot_id']." target=_balnk>".self::$language['snapshot']."</a></div>
		  </span>
	  </div>";	
