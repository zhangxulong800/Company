...
$sql="select `id`,`title`,`w_price`,`max_price`,`introducer_rate` from ".self::$table_pre."goods where `shop_id`=".SHOP_ID;
...
$order=" order by `introducer_rate` desc,`id` desc";
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	$sum_sql=$sql.$where;
	$sum_sql=str_replace(" `id`,`title`,`w_price`,`max_price`,`introducer_rate` "," count(id) as c ",$sum_sql);
...
foreach($r as $v){
  ...
  <td><input type='text' name='introducer_rate_".$v['id']."' id='introducer_rate_".$v['id']."' value='".$v['introducer_rate']."' class='introducer_rate' /> %</td>
