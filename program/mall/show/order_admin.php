...
if(intval(@$_GET['id'])!=0){
	...
	echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?monxin=mall.order_admin">'.self::$language['pages']['mall.order_admin']['name'].'</a><span class=text>'.$_GET['id'].'</span></div>';
}
...
$module['days']='<a href="./index.php?monxin=mall.order_admin&start_time='.$today.'&end_time='.$today.'" se="'.$today.'-'.$today.'">'.self::$language['today'].'</a>';
$module['days'].='<a href="./index.php?monxin=mall.order_admin&start_time='.$yesterday.'&end_time='.$yesterday.'" se="'.$yesterday.'-'.$yesterday.'">'.self::$language['yesterday'].'</a>';
$module['days'].='<a href="./index.php?monxin=mall.order_admin&start_time='.$days_7.'&end_time='.$today.'" se="'.$days_7.'-'.$today.'">'.self::$language['days_7'].'</a>';
$module['days'].='<a href="./index.php?monxin=mall.order_admin&start_time='.$days_30.'&end_time='.$today.'" se="'.$days_30.'-'.$today.'">'.self::$language['days_30'].'</a>';
...
foreach($r as $v){
	...
	if($v['shop_credits_money']>0){$v['credits_remark']='<div class=credits_remark>'.self::$language['use_shop_credits'].':'.$v['shop_credits'].','.self::$language['deduction'].$v['shop_credits_money'].self::$language['yuan'].'</div>';}else{$v['credits_remark']='';}
	...
	foreach($r2 as $v2){
		...
		if($_COOKIE['monxin_device']=='pc'){
        	...
        	<span class=icon>
                ...
                <div class=price>".self::$language['price'].":".$o_price.str_replace('.00','',$v2['price']).self::$language['yuan']." &nbsp; ".self::$language['quantity'].":".self::format_quantity($v2['quantity']).$v2['unit'].$refund.str_replace('d_id','d_id='.$v2['id'],$edit_quantity)." <a href=./index.php?monxin=mall.goods_snapshot&id=".$v2['snapshot_id']." target=_balnk>".self::$language['snapshot']."</a></div>
            </span>
        </div>";
		}
