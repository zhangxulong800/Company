...
$sql="select * from ".self::$table_pre."finance";
...
if(@$_GET['method']!=''){
	if(intval($_GET['method'])==0){$where.=" and `money`<0";}else{$where.=" and `money`>0";}
}
...
if($_GET['search']!=''){$where.=" and (`money` like '%".$_GET['search']."%' || `reason` like '%".$_GET['search']."%' || `operator` like '%".$_GET['search']."%' || `shop_name` like '%".$_GET['search']."%')";}
if(@$_GET['order']==''){
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	...
	$sum_sql=str_replace("_finance and","_finance where",$sum_sql);
	$r=$pdo->query($sum_sql,2)->fetch(2);
	$sum=$r['c'];
	
	$income=$sql.' and `money`>0'.str_replace(' and `money`<0','',$where);
	$income=str_replace(" * "," sum(money) as c ",$income);
	$income=str_replace("_finance and","_finance where",$income);
	...
	$expenditure=str_replace("_finance and","_finance where",$expenditure);
...
$sql=str_replace("_finance and","_finance where",$sql);
//echo($sql);
//exit();
$r=$pdo->query($sql,2);
$list='';
foreach($r as $v){
	...
	<td><span class=type>".self::$language['finance_type'][$v['type']]."</span></td>
...
if(@$_GET['shop_id']!=''){
	echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?monxin=mall.finance">'.self::$language['pages']['mall.finance']['name'].'</a><span class=text>'.$v['shop_name'].'</span></div>';
}
...
function get_finance_method($pdo,$language){
...
function get_finance_type_option($pdo,$language){
	$list="<option value=''>".$language['please_select']."</option>";
	foreach($language['finance_type'] as $k=>$v){
		if($k==0 || $k==5){
			$list.='<option value="'.$k.'">'.$v.'</option>';
...
$module['filter']=get_finance_method($pdo,self::$language);
$module['filter'].=get_finance_type($pdo,self::$language);

$module['finance_method']=get_finance_method_option($pdo,self::$language);
$module['finance_type']=get_finance_type_option($pdo,self::$language);
