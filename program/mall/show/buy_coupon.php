...
$sql="select * from ".self::$table_pre."buy_coupon where `shop_id`=".SHOP_ID." order by `id` desc";
...
foreach($r as $v){
	$list.="<tr id='tr_".$v['id']."'>
	  ...
	  <td class=operation_td><a href='./index.php?monxin=mall.buy_coupon_list&c_id=".$v['id']."' target=_blank lass='view'>".self::$language['view']."</a><a href='".$module['action_url']."&act=export_csv&id=".$v['id']."'  target=_blank  class='export'>".self::$language['export']."</a><a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
	</tr>
";	
