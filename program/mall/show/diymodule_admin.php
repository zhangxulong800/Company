...
$sql="select `id`,`title`,`sequence`,`width`,`height`,`time` from ".self::$table_pre."module where `shop_id`=".SHOP_ID;
...
$limit=" limit ".($current_page-1)*$page_size.",".$page_size;
	$sum_sql=$sql.$where;
	$sum_sql=str_replace(" `id`,`title`,`sequence`,`width`,`height`,`time` "," count(id) as c ",$sum_sql);
...
foreach($r as $v){
  ...
  <td class=operation_td><a href='#' onclick='return update(".$v['id'].")'  class='submit'>".self::$language['submit']."</a> <a href='index.php?monxin=".$class.".diymodule_edit&id=".$v['id']."' class='edit'>".self::$language['edit']."</a> <a href='#' onclick='return del(".$v['id'].")'  class='del'>".self::$language['del']."</a> <span id=state_".$v['id']." class='state'></span></td>
</tr>
";	
}
