...
$sql="select * from ".self::$table_pre."type_option where `type_id`='".$type."' order by `sequence` desc,`id` asc";
$r=$pdo->query($sql,2);
$list='';
foreach($r as $v){
	$list.="<tr id='tr_".$v['id']."'>
	  ...
	  <td><input type='text' name='shop_".$v['id']."' id='shop_".$v['id']."' value='".$v['shop_id']."'  class='shop' /></td>
...
$sql="select `specifications_source`,`parent`,`name`,`color`,`diy_option`,`option_name` from ".self::$table_pre."type where `id`=".$type;
$r=$pdo->query($sql,2)->fetch(2);
$module['color']=$r['color'];
$module['diy_option']=$r['diy_option'];
$module['option_name']=$r['option_name'];
$module['specifications_source']=$r['specifications_source'];
echo '<div  style="display:none;" id="user_position_append">'.$this->get_type_user_position($pdo,$r['parent']).'<span class=text>'.$r['name'].' '.self::$language['pages']['mall.type_option']['name'].'</span></div>';
