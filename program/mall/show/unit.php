...
$module['module_name']=str_replace("::","_",$method);
		$sql="select * from ".self::$table_pre."unit order by `sequence` desc,`id` asc";
		$r=$pdo->query($sql,2);
		$list='';
		foreach($r as $v){
			$list.="<tr id='tr_".$v['id']."'>
				<td><input type='text' name='name_".$v['id']."' id='name_".$v['id']."' value='".$v['name']."'  class='name' /></td>
				<td>=<input type='text' name='gram_".$v['id']."' id='gram_".$v['id']."' value='".$v['gram']."'  class='gram' />".self::$language['gram']."</td>
