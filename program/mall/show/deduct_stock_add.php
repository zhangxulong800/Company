...
if(isset($module['data']['id'])){
	...
	$sql="select `reason` from ".self::$table_pre."goods_deduct_stock where `goods_id`=".$id." group by `reason` order by `id` desc limit 0,10";
	$r=$pdo->query($sql,2);
	$i=0;
	foreach($r as $v){
		if($v['reason']==self::$language['auto_deleave_big_box']){continue;}
	...
	if($i<10){
		$sql="select `reason` from ".self::$table_pre."goods_deduct_stock where `goods_id`!=".$id." group by `reason` order by `id` desc limit 0,".(10-$i);
		$r=$pdo->query($sql,2);
		foreach($r as $v){
			if($v['reason']==self::$language['auto_deleave_big_box']){continue;}
			$v['reason']=de_safe_str($v['reason']);
