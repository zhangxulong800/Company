...
$sql="select `id`,`name`,`span_circle` from ".self::$table_pre."shop";
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	$sum_sql=$sql.$where;
	$sum_sql=str_replace(" `id`,`name`,`span_circle` "," count(id) as c ",$sum_sql);
...
foreach($r as $v){
	...
	$list.="<tr id='tr_".$v['id']."'>
	  <td>".$v['name']."</td>
	  <td><input type=checkbox monxin_value=".$v['span_circle']."></td>
	  <td class=operation_td><a href='#' class='submit'>".self::$language['submit']."</a> <span id=state_".$v['id']." class='state'></span></td>
	</tr>
";	
