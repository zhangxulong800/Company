	...
	if($v['option_enable']==1){
		...
		foreach($r2 as $v2){
			...
			if(strlen($v2['barcode'])>=$min && strlen($v2['barcode'])<$max ){
				$pages[$page].='<div class=goods><img src="http://'.self::$config['web']['domain'].'/plugin/barcode/buildcode.php?codebar=BCGcode128&text='.$v2['barcode'].'" /><span>'.self::get_type_option_name($pdo,$v2['option_id']).' '.$v2['color_name'].' '.$v['title'].'</span></div>';
				$i++;
			}
		}
	}else{
		...
		if(strlen($v['bar_code'])>=$min && strlen($v['bar_code'])<=$max ){
			$pages[$page].='<div class=goods><img src="http://'.self::$config['web']['domain'].'/plugin/barcode/buildcode.php?codebar=BCGcode128&text='.$v['bar_code'].'" /><span>'.$v['title'].'</span></div>';
			$i++;
		}
	}
...
if($i<=30){$pages[$page]='<div class=print_content>'.$pages[$page].'</div>';}
