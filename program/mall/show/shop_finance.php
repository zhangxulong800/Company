...
$sql="select * from ".self::$table_pre."shop_finance where `shop_id`=".SHOP_ID;
...
if($_GET['search']!=''){$where.=" and (`money` like '%".$_GET['search']."%' || `reason` like '%".$_GET['search']."%')";}
if(@$_GET['order']==''){
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	...
	$sum_sql=str_replace("_shop_finance and","_shop_finance where",$sum_sql);
	$r=$pdo->query($sum_sql,2)->fetch(2);
	$sum=$r['c'];
	
	$income=$sql.' and `money`>0'.str_replace(' and `money`<0','',$where);
	$income=str_replace(" * "," sum(money) as c ",$income);
	$income=str_replace("_shop_finance and","_shop_finance where",$income);
	...
	$expenditure=str_replace("_shop_finance and","_shop_finance where",$expenditure);
...
$sql=str_replace("_shop_finance and","_shop_finance where",$sql);
//echo($sql);
//exit();
$r=$pdo->query($sql,2);
$list='';
foreach($r as $v){
	...
	<td><span class=type>".self::$language['finance_type_shop'][$v['type']]."</span></td>
...
function get_shop_finance_method($pdo,$language){
...
function get_shop_finance_type_option($pdo,$language){
	$list="<option value=''>".$language['please_select']."</option>";
	foreach($language['finance_type_shop'] as $k=>$v){
		$list.='<option value="'.$k.'">'.$v.'</option>';
	}
...
$module['filter']=get_shop_finance_method($pdo,self::$language);
$module['filter'].=get_shop_finance_type($pdo,self::$language);

$module['finance_method']=get_shop_finance_method_option($pdo,self::$language);
$module['finance_type']=get_shop_finance_type_option($pdo,self::$language);
