...
$module['data']['w_price']=money_to_credits(self::$config['credits_set']['rate'],self::$config['pay_mode'],$module['data']['w_price']);
$module['data']['min_price']=money_to_credits(self::$config['credits_set']['rate'],self::$config['pay_mode'],$module['data']['min_price']);
$module['data']['max_price']=money_to_credits(self::$config['credits_set']['rate'],self::$config['pay_mode'],$module['data']['max_price']);

$sql="select * from ".self::$table_pre."shop where `id`=".SHOP_ID;
$shop_info=$pdo->query($sql,2)->fetch(2);
...
if($module['data']['state']==1){
	...
	$r=$pdo->query($sql,2)->fetch(2);
	$module['data']['deposit']=money_to_credits(self::$config['credits_set']['rate'],self::$config['pay_mode'],$r['deposit']);	
	$module['data']['reduction']=money_to_credits(self::$config['credits_set']['rate'],self::$config['pay_mode'],$r['reduction']);	
...
if($module['data']['option_enable']){
	...
	if($module['data']['state']==1){
		...
		<div class=old_price><span class=price_label>'.self::$language['price'].'</span><span class=value  old="'.$temp2.'">'.$temp2.'</span><span class=money_symbol>'.self::$language['yuan'].'</span></div>
		<div class=pre_div>
			<div class=pre_price><span class=price_label>'.self::$language['pre_price'].'</span><span class=price_value old="'.$temp.'">'.$temp.'</span><span class=money_symbol>'.self::$language['yuan'].'</span></div>
			<div class=pre_deposit><span class=price_label>'.self::$language['deposit2'].'</span><span class=deposit_value>'.$module['data']['deposit'].'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=reduction_div>'.self::$language['deposit2'].self::$language['reduce'].'<span class=reduction> '.$module['data']['reduction'].'<span class=money_symbol>'.self::$language['yuan'].'</span></span></span><a href="#" class=pre_sale_rules>'.self::$language['pre_sale_rules'].'</a></div>
		</div>
		</div>';		
	}else{
		...
		if(isset($_SESSION['monxin'][SHOP_ID.'_discount']) and ($_SESSION['monxin'][SHOP_ID.'_discount']<10 || $goods_group_discount<10) and ($_SESSION['monxin'][SHOP_ID.'_discount'] < $discount || $goods_group_discount< $discount)  and $module['data']['sales_promotion']){
			...
			$module['price_div_html']='<div id=have_discount>
								<div class=normal_price><span class=price_label>'.self::$language['normal_price'].'</span><span class=price_value>'.$temp2.'</span><span class=money_symbol>'.self::$language['yuan'].'</span></div>
								<div class=discount_price><span class=price_label>'.$_SESSION['monxin'][SHOP_ID.'_shop_group'].self::$language['price2'].':</span><span class=price_value>'.$temp.'</span><span class=money_symbol>'.self::$language['yuan'].'</span>  <span class=discount_value><span class=discount_number>'.$goods_group_discount.'</span>'.self::$language['discount'].'</span></div>
		...
		//----否则	
			...
			if($discount==10){
				...
				$module['price_div_html']='<div id=have_discount>
									<div class=discount_price><span class=price_label>'.self::$language['limited_discount'].'</span><span class=price_value>'.$temp.'</span><span class=money_symbol>'.self::$language['yuan'].'</span> <span class=discount_value><span class=discount_number>'.$discount.'</span>'.self::$language['discount'].'</span><span class=discount_time>'.self::$language['the_last'].$remain_time.'</span></div>
									<div class=normal_price><span class=price_label>'.self::$language['normal_price'].'</span><span class=price_value>'.$temp2.'</span><span class=money_symbol>'.self::$language['yuan'].'</span></div>
								</div>';	
			}
		}		
	...
	if($module['data']['state']==1){
		...
		$module['price_div_html']='<div id=pre_sale>
		<div class=old_price><span class=price_label>'.self::$language['price'].'</span><span class=value>'.$module['data']['w_price'].'</span><span class=money_symbol>'.self::$language['yuan'].'</span></div>
		<div class=pre_div>
			<div class=pre_price><span class=price_label>'.self::$language['pre_price'].'</span><span class=price_value>'.sprintf("%.2f",$module['data']['w_price']*$module['data']['pre_discount']/10).'</span><span class=money_symbol>'.self::$language['yuan'].'</span></div>
			<div class=pre_deposit><span class=price_label>'.self::$language['deposit2'].'</span><span class=deposit_value>'.$module['data']['deposit'].'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=reduction_div>'.self::$language['deposit2'].self::$language['reduce'].'<span class=reduction> '.$module['data']['reduction'].'<span class=money_symbol>'.self::$language['yuan'].'</span></span></span><a href="#" class=pre_sale_rules>'.self::$language['pre_sale_rules'].'</a></div>
		</div>
		</div>';		
	}else{
		...
		//----否则			
			if($discount==10){
				$module['price_div_html']='<div id=no_discount><span class=price_label>'.self::$language['goods_price'].'</span><span class=price_value>'.$module['data']['w_price'].'</span><span class=money_symbol>'.self::$language['yuan'].'</span></div>';	
			}else{
				$remain_time='';
				$time=$end_time-$time;
				if($time>86400){
				...
				$module['price_div_html']='<div id=have_discount>
									<div class=discount_price><span class=price_label>'.self::$language['limited_discount'].'</span><span class=price_value>'.sprintf("%.2f",($module['data']['w_price']*$discount/10)).'</span><span class=money_symbol>'.self::$language['yuan'].'</span> <span class=discount_value><span class=discount_number>'.$discount.'</span>'.self::$language['discount'].'</span><span class=discount_time>'.self::$language['the_last'].$remain_time.'</span></div>
									<div class=normal_price><span class=price_label>'.self::$language['normal_price'].'</span><span class=price_value>'.$module['data']['w_price'].'</span><span class=money_symbol>'.self::$language['yuan'].'</span></div>
								</div>';	
			}
		}
	}
...
$module['log_div_html']='<div class=line_'.$right_info_count.'>'.$right_info.'<a class=credits><span class=m_label>'.str_replace('{v}',$shop_info['credits_rate'],self::$language['shop_credits_rate']).'</span></a></div>';
	...
	foreach($r as $v){
		...
		foreach($r2 as $v2){
			...
			if($v2['option_enable']==0){
				$v2['w_price']=money_to_credits(self::$config['credits_set']['rate'],self::$config['pay_mode'],$v2['min_price']);
				$price_sum+=$v2['w_price'];
				$w_price='<span class=money_value>'.$v2['w_price'].'</span><span class=money_symbol>'.self::$language['yuan'].'</span>';
			}else{
				...
				if($v2['min_price']==$v2['max_price']){
					$w_price='<span class=money_value>'.$v2['min_price'].'</span><span class=money_symbol>'.self::$language['yuan'].'</span>';
				}else{
					$w_price='<span class=money_value>'.$v2['min_price'].'-'.$v2['max_price'].'</span><span class=money_symbol>'.self::$language['yuan'].'</span>';
				}
			}
		...
		<div class=normal_price><span class=m_label>'.self::$language['normal_price'].'</span><span class=value>'.$price_sum.'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=more_than>'.self::$language['more_than'].'</span></div>
		<div class=package_price><span class=m_label>'.self::$language['package_price'].'</span><span class=value>'.sprintf("%.2f",($price_sum*$v['discount']/10)).'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=more_than>'.self::$language['more_than'].'</span></div>
		<div class=save_money><span class=m_label>'.self::$language['save_money'].'</span><span class=value>'.sprintf("%.2f",($price_sum-($price_sum*$v['discount']/10))).'</span><span class=money_symbol>'.self::$language['yuan'].'</span><span class=more_than>'.self::$language['more_than'].'</span></div>
