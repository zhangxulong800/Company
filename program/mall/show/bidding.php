...
$sql="select `gid`,`shop_id`,sum(money) as s_money,count(id) as click from ".self::$table_pre."bidding group by `gid` order by `id` desc";
if(isset($_GET['shop_id'])){
	...
	if($shop_id>0){
		...
		$shop_name=$r2['name'];
		echo '<div  style="display:none;" id="user_position_append"><a href="./index.php?monxin=mall.bidding">'.self::$language['pages']['mall.bidding']['name'].'</a><span class=text>'.$shop_name.'</span></div>';
...
foreach($r as $v){
	...
	<td>".$v['click']."</td>
	<td><a href=./index.php?monxin=mall.bidding_detail&gid=".$v['gid']." target=_blank>".$v['s_money']."</a></td>
	<td><a href=./index.php?monxin=mall.bidding&shop_id=".$v['shop_id']." target=_blank>".$shop_name."</a></td>
</tr>
";	
}
