...
$id=intval(@$_GET['id']);
$module['css']='';
$sql="select * from ".self::$table_pre."goods_loss where `shop_id`='".SHOP_ID."'";
if($id>0){
	$sql.=" and `goods_id`=".$id;
...
$limit=" limit ".($_GET['current_page']-1)*$page_size.",".$page_size;
	...
	$sum_sql=str_replace("_goods_loss and","_goods_loss where",$sum_sql);
 	$r=$pdo->query($sum_sql,2)->fetch(2);
	$sum=$r['c'];
$sql=$sql.$where.$order.$limit;
$sql=str_replace("_goods_loss and","_goods_loss where",$sql);
//echo($sql);
//exit();
$r=$pdo->query($sql,2);
$list='';
