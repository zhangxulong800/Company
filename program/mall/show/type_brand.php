...
$sql="select * from ".self::$table_pre."type_brand where `type_id`='".$type."' order by `sequence` desc,`id` asc";
$r=$pdo->query($sql,2);
$list='';
foreach($r as $v){
	$list.="<tr id='tr_".$v['id']."'>
...
$sql="select `brand_source`,`parent`,`name` from ".self::$table_pre."type where `id`=".$type;
$r=$pdo->query($sql,2)->fetch(2);
$module['brand_source']=$r['brand_source'];

echo '<div  style="display:none;" id="user_position_append">'.$this->get_type_user_position($pdo,$r['parent']).'<span class=text>'.$r['name'].' '.self::$language['pages']['mall.type_brand']['name'].'</span></div>';
