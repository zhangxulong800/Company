...
switch($act){
	...
	case 'paid':
		if($r['state']!=0){echo '<div class=return_false>'.self::$language['state'].':'.self::$language['order_state'][$r['state']]." ".self::$language['inoperable'].'</div>';return false;}
		$o=$r;
		...
		if(is_dir('./scanpay_type/')){
			foreach(self::$language['scan_pay_account_type_option'] as $k=>$v){
				...
				$r=$pdo->query($sql,2)->fetch(2);
				if($r['id']!=''){
					$list.='<a href="'.$k.'"  account_id='.$r['id'].' lhref="./index.php?monxin=mall.checkout_search&act=scanpay&id='.$r['id'].'&user='.$o['buyer'].'&money='.$o['actual_money'].'">'.self::$language['scan_pay_account_type_option'][$k].'('.self::$language['scan_pay_is_web_option'][0].')</a>';
				}else{
					$sql="select * from ".$pdo->sys_pre."scanpay_account where `is_web`=1 and `type`='".$k."' and `state`=1  order by `sequence` desc limit 0,1";
					$r=$pdo->query($sql,2)->fetch(2);
					if($r['id']!=''){
						$list.='<a href="'.$k.'"  account_id='.$r['id'].'  lhref="./index.php?monxin=mall.checkout_search&act=scanpay&id='.$r['id'].'&user='.$o['buyer'].'&money='.$o['actual_money'].'">'.self::$language['scan_pay_account_type_option'][$k].'('.self::$language['scan_pay_is_web_option'][1].')</a>';
						}
		...
		*/
		...
		foreach($pay as $k=>$v){
			$list.='<a href="'.$k.'">'.self::$language['pay_method'][$k].'</a>';
		}
			$r=$o;
		if($r['buyer']!=''){
			$sql="select `money` from ".$pdo->index_pre."user where `username`='".$r['buyer']."' limit 0,1";
			$u=$pdo->query($sql,2)->fetch(2);
			if($u['money']>=$r['actual_money']){$list='<a href="balance">'.self::$language['pay_method']['balance'].'</a>'.$list;}
			
			$sql="select `balance` from ".self::$table_pre."shop_buyer where `shop_id`=".SHOP_ID." and `username`='".$r['buyer']." limit 0,1'";
			$b=$pdo->query($sql,2)->fetch(2);
			if($b['money']>=$r['actual_money']){$list='<a href="shop_balance">'.self::$language['pay_method']['shop_balance'].'</a>'.$list;}
		}
		
		
		$module['html']='<div class=pay_method_div><div class=title>'.self::$language['pay_method_str'].'</div><div class=pay_method>'.$list.'</div><div class=input_div><div class=transaction_password_div>'.self::$language['transaction_password'].' <input type=password class=transaction_password /></div><div class=reference_number_div>'.self::$language['reference_number'].': <input type=text class=reference_number /></div></div><div class=submit_div><a href=# class=submit>'.self::$language['submit'].'</a> <span class=state></span></div></div>';
		break;	
}
