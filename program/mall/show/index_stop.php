...
/*
//update goods_specifications type
$sql="select `id`,`goods_id` from ".self::$table_pre."goods_specifications";
$r=$pdo->query($sql,2);
foreach($r as $v){
	$sql="select `type` from ".self::$table_pre."goods where `id`=".$v['goods_id'];	
	$r2=$pdo->query($sql,2)->fetch(2);
	$sql="update ".self::$table_pre."goods_specifications set `type`=".$r2['type']." where `id`=".$v['id'];
	$pdo->exec($sql);
}


//update goods_attribute type
$sql="select `id`,`goods_id` from ".self::$table_pre."goods_attribute";
$r=$pdo->query($sql,2);
foreach($r as $v){
	$sql="select `type` from ".self::$table_pre."goods where `id`=".$v['goods_id'];	
	$r2=$pdo->query($sql,2)->fetch(2);
	$sql="update ".self::$table_pre."goods_attribute set `type`=".$r2['type']." where `id`=".$v['id'];
	$pdo->exec($sql);
}
*/

function update_mall_goods_monthly($pdo,$table_pre){
	$sql="select `id` from ".$table_pre."goods where `state`=1";
	$r=$pdo->query($sql,2);
